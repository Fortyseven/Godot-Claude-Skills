...
When the user asks about a **specific term, annotation, function, or pattern** (e.g., `@icon`, `@export_range`, `class_name`, `preload`, `EditorImportPlugin`), **first** load `references/KEYWORD_INDEX.md`. This file maps terms directly to the files containing them. Load the listed file(s) to answer the question.
