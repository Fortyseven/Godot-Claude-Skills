# Godot 4 GDScript Tutorials — Assets Pipeline (Part 1)
...
This means that when trying to access imported assets through code, you need to use the [Resource Loader](../godot_gdscript_core.md) as it will automatically take into account where the internal files are saved. If you try and access an imported asset using the [FileAccess](../godot_gdscript_filesystem.md) class, it will work in the editor, but **it will break in the exported project**.

However, the [Resource Loader](../godot_gdscript_core.md) cannot access non-imported files. Only the [FileAccess](../godot_gdscript_filesystem.md) class can.
...
- **Save to File:** Saves the [Mesh](../godot_gdscript_rendering.md) _resource_ to an external file (this isn't a scene file). You generally don't need to use this for placing the mesh in a 3D scene – instead, you should instance the 3D scene directly. However, having direct access to the Mesh resource is useful for specific nodes, such as [MeshInstance3D](../godot_gdscript_nodes_3d.md), [MultiMeshInstance3D](../godot_gdscript_nodes_3d.md), [GPUParticles3D](../godot_gdscript_misc.md) or [CPUParticles3D](../godot_gdscript_misc.md). - You will also need to specify an output file path using the option that appears after enabling **Save to File**. It's recommended to use the `.res` output file extension for smaller file sizes and faster loading speeds, as `.tres` is inefficient for writing large amounts of data.
...
Several extra options are available for the generated [AnimationPlayer](../godot_gdscript_resources.md) nodes, as well as their individual animations when they're selected in the **Scene** tab.
...
- **Light Baking:** Configures the meshes' [global illumination mode](../godot_gdscript_misc.md) in the 3D scene. If set to **Static Lightmaps**, sets the meshes' GI mode to **Static** and generates UV2 on import for [lightmap baking](tutorials_3d.md).
...
- **Use Named Skins:** If checked, use named [Skins](../godot_gdscript_misc.md) for animation. The [MeshInstance3D](../godot_gdscript_nodes_3d.md) node contains 3 properties of relevance here: a skeleton NodePath pointing to the Skeleton3D node (usually `..`), a mesh, and a skin:

- The [Skeleton3D](../godot_gdscript_nodes_3d.md) node contains a list of bones with names, their pose and rest, a name and a parent bone.
...
- **Custom Properties:** If checked, imports custom properties from Blender as glTF extras. This data can then be used from an editor plugin that uses [GLTFDocument.register_gltf_document_extension()](../godot_gdscript_misc.md), which can set node metadata on import (among other use cases).
...
Click **Reimport** and restart the editor when prompted. After restarting, the glTF file will be imported as an [AnimationLibrary](../godot_gdscript_resources.md) instead of a [PackedScene](../godot_gdscript_resources.md). This animation library can then be referenced in an [AnimationPlayer](../godot_gdscript_resources.md) node.
