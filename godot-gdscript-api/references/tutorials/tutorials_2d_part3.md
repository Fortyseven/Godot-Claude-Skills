# Godot 4 GDScript Tutorials — 2D (Part 3)
...
Godot provides two different nodes for 2D particles, [GPUParticles2D](../godot_gdscript_nodes_2d.md) and [CPUParticles2D](../godot_gdscript_nodes_2d.md). GPUParticles2D is more advanced and uses the GPU to process particle effects. CPUParticles2D is a CPU-driven option with near-feature parity with GPUParticles2D, but lower performance when using large amounts of particles. On the other hand, CPUParticles2D may perform better on low-end systems or in GPU-bottlenecked situations.

While GPUParticles2D is configured via a [ParticleProcessMaterial](../godot_gdscript_rendering.md) (and optionally with a custom shader), the matching options are provided via node properties in CPUParticles2D (with the exception of the trail settings).
