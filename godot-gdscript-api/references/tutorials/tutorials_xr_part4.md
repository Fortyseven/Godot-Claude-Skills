# Godot 4 GDScript Tutorials — Xr (Part 4)
...
In the XR action map, actions are the entities that your game/application will interact with. For instance, we can define an action `Shoot` and the input bound to that action will trigger the `button_pressed` signal on the relevant [XRController3D](../godot_gdscript_misc.md) node in your scene with `Shoot` as the `name` parameter of the signal.

You can also poll the current state of an action. [XRController3D](../godot_gdscript_misc.md) for instance has an `is_button_pressed` method.
...
These two are special as they are the only ones that are interchangeable. OpenXR will handle conversions between `Bool` and `Float` inputs and actions. You can get the value of a `Float` type action by calling the method `get_float` on your [XRController3D](../godot_gdscript_misc.md) node. It emits the `input_float_changed` signal when changed.
...
The `Vector2` type defines the input as an axis input. Touchpads, thumbsticks and similar inputs are exposed as vectors. You can get the value of a `Vector2` type action by calling the method `get_vector2` on your [XRController3D](../godot_gdscript_misc.md) node. It emits the `input_vector2_changed` signal when changed.

The `Pose` type defines a spatially tracked input. Multiple "pose" inputs are available in OpenXR: `aim`, `grip` and `palm`. Your [XRController3D](../godot_gdscript_misc.md) node is automatically positioned based on the pose action assigned to `pose` property of this node. More about poses later.
...
Now note that we don't distinguish between the left and right hand. This is something that is determined at the next stage. We've implemented the action system in such a way that you can bind the same action to both hands. The appropriate [XRController3D](../godot_gdscript_misc.md) node will emit the signal.
...
When adding custom full screen effects to your XR application, one approach is using a full screen quad and applying effects to that quad's shader. Add a [MeshInstance3D](../godot_gdscript_nodes_3d.md) node to your scene as a child of your [XRCamera3D](../godot_gdscript_misc.md), and set the `mesh` property to a [QuadMesh](../godot_gdscript_rendering.md). Set the width and height of the quad to `2`.
...
In traditional first person games a player is represented by a [CharacterBody3D](../godot_gdscript_nodes_3d.md) node. This node is moved by processing traditional controller, mouse or keyboard input. A camera is attached to this node at a location roughly where the player's head will be.

Applying this model to the XR setup, we add an [XROrigin3D](../godot_gdscript_misc.md) node as a child of the character body, and add an [XRCamera3D](../godot_gdscript_misc.md) as a child of the origin node. At face value this seems to work. However, upon closer examination this model does not take into account that there are two forms of movement in XR. The movement through controller input, and the physical movement of the player in the real world.
...
func _process_on_physical_movement(delta):
  ...
  # Check if we managed to move all the way, ignoring
# ...
```
...
func _process_on_physical_movement(delta) -> bool:
  ...
  origin_node.transform = Transform3D
# ...
```
