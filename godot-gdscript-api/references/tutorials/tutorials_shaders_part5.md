# Godot 4 GDScript Tutorials — Shaders (Part 5)
...
This way, the complex calculations happen only in the cubemap pass, which can be optimized by setting the sky's [process mode](../godot_gdscript_misc.md) and the [radiance size](../godot_gdscript_misc.md) to get the desired balance between performance and visual fidelity.
...
> **Warning:** The `light()` function won't be run if the `vertex_lighting` render mode is enabled, or if [Rendering > Quality > Shading > Force Vertex Shading](../godot_gdscript_misc.md) is enabled in the Project Settings. (It's enabled by default on mobile platforms.)
