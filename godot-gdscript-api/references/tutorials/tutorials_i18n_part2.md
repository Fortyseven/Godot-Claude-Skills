# Godot 4 GDScript Tutorials — I18N (Part 2)
...
> **See also:** You can see how pseudolocalization works in action using the [Pseudolocalizaton demo project](https://github.com/godotengine/godot-demo-projects/tree/master/gui/pseudolocalization).
...
- `replace_with_accents`: Replaces all characters in the string with their accented variants. _"The quick brown fox jumped over the lazy dog"_ will be converted to _"Ŧh̀é q́üíćḱ ḅŕôŵή f́ôx́ ǰüm̀ṕéd́ ôṽéŕ ŧh̀é łáźý d́ôǵ"_ when this setting is enabled. This can be used to spot untranslated strings that won't have accents, but is also useful to check for missing glyphs in the font(s) used by the project.
...
Pseudolocalization can be toggled at runtime using the [pseudolocalization_enabled](../godot_gdscript_misc.md) property in TranslationServer. However, if runtime configuration of pseudolocalization properties is required, they can be directly configured using [ProjectSettings.set_setting(property, value)](../godot_gdscript_filesystem.md) and then calling [TranslationServer.reload_pseudolocalization()](../godot_gdscript_misc.md) which reparses the pseudolocalization properties and reloads the pseudolocalization. The following code snippet shall turn on `replace_with_accents` and `double_vowels` properties and then call `reload_pseudolocalization()` for the changes to get reflected:
