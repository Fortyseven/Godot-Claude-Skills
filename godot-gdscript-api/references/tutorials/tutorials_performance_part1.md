# Godot 4 GDScript Tutorials — Performance (Part 1)
...
One quirk of the [SceneTree](../godot_gdscript_core.md) is that you can sometimes get much better performance by removing nodes from the SceneTree, rather than by pausing or hiding them. You don't have to delete a detached node. You can for example, keep a reference to a node, detach it from the scene tree using [Node.remove_child(node)](../godot_gdscript_misc.md), then reattach it later using [Node.add_child(node)](../godot_gdscript_misc.md). This can be very useful for adding and removing areas from a game, for example.
...
The Godot renderer is a little different to what is out there. It's designed to minimize GPU state changes as much as possible. [StandardMaterial3D](../godot_gdscript_rendering.md) does a good job at reusing materials that need similar shaders. If custom shaders are used, make sure to reuse them as much as possible. Godot's priorities are:
...
You can increase performance in a fill rate-limited project by reducing the amount of work the GPU has to do. You can do this by simplifying the shader (perhaps turn off expensive options if you are using a [StandardMaterial3D](../godot_gdscript_rendering.md)), or reducing the number and size of textures used. Also, when using non-unshaded particles, consider forcing vertex shading in their material to decrease the shading cost.
...
The [VisibleOnScreenEnabler3D](../godot_gdscript_misc.md) and [VisibleOnScreenNotifier3D](../godot_gdscript_misc.md) nodes can be useful for this purpose.
