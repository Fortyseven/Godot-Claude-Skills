# Godot 4 GDScript Tutorials — Rendering (Part 2)
...
Think of a [Viewport](../godot_gdscript_rendering.md) as a screen onto which the game is projected. In order to see the game, we need to have a surface on which to draw it. That surface is the Root Viewport.

[SubViewports](../godot_gdscript_rendering.md) are a kind of Viewport that can be added to the scene so that there are multiple surfaces to draw on. When we are drawing to a SubViewport, we call it a render target. We can access the contents of a render target by accessing its corresponding [texture](../godot_gdscript_misc.md). By using a SubViewport as render target, we can either render multiple scenes simultaneously or we can render to a [ViewportTexture](../godot_gdscript_rendering.md) which is applied to an object in the scene, for example a dynamic skybox.

[SubViewports](../godot_gdscript_rendering.md) have a variety of use cases, including:
...
Another kind of Viewports in Godot are [Windows](../godot_gdscript_ui_controls.md). They allow their content to be projected onto a window. While the Root Viewport is a Window, they are less flexible. If you want to use the texture of a Viewport, you'll be working with [SubViewports](../godot_gdscript_rendering.md) most of the time.

### Input

[Viewports](../godot_gdscript_rendering.md) are also responsible for delivering properly adjusted and scaled input events to their children nodes. By default [SubViewports](../godot_gdscript_rendering.md) don't automatically receive input, unless they receive it from their direct [SubViewportContainer](../godot_gdscript_ui_controls.md) parent node. In this case, input can be disabled with the [Disable Input](../godot_gdscript_misc.md) property.

For more information on how Godot handles input, please read the [Input Event Tutorial](tutorials_inputs.md).

### Listener

Godot supports 3D sound (in both 2D and 3D nodes). More on this can be found in the [Audio Streams Tutorial](tutorials_audio.md). For this type of sound to be audible, the [Viewport](../godot_gdscript_rendering.md) needs to be enabled as a listener (for 2D or 3D). If you are using a [SubViewport](../godot_gdscript_rendering.md) to display your [World3D](../godot_gdscript_misc.md) or [World2D](../godot_gdscript_misc.md), don't forget to enable this!

### Cameras (2D & 3D)

When using a [Camera3D](../godot_gdscript_nodes_3d.md) or [Camera2D](../godot_gdscript_nodes_2d.md), it will always display on the closest parent [Viewport](../godot_gdscript_rendering.md) (going towards the root). For example, in the following hierarchy:

`CameraA` will display on the Root [Viewport](../godot_gdscript_rendering.md) and it will draw `MeshA`. `CameraB` will be captured by the [SubViewport](../godot_gdscript_rendering.md) along with `MeshB`. Even though `MeshB` is in the scene hierarchy, it will still not be drawn to the Root Viewport. Similarly, `MeshA` will not be visible from the SubViewport because SubViewports only capture nodes below them in the hierarchy.

There can only be one active camera per [Viewport](../godot_gdscript_rendering.md), so if there is more than one, make sure that the desired one has the [current](../godot_gdscript_misc.md) property set, or make it the current camera by calling:

```gdscript
camera.make_current()
```

By default, cameras will render all objects in their world. In 3D, cameras can use their [cull_mask](../godot_gdscript_misc.md) property combined with the [VisualInstance3D's](../godot_gdscript_nodes_3d.md) [layer](../godot_gdscript_misc.md) property to restrict which objects are rendered.

### Scale & stretching

[SubViewports](../godot_gdscript_rendering.md) have a [size](../godot_gdscript_misc.md) property, which represents the size of the SubViewport in pixels. For SubViewports which are children of [SubViewportContainers](../godot_gdscript_ui_controls.md), these values are overridden, but for all others, this sets their resolution.

It is also possible to scale the 2D content and make the [SubViewport](../godot_gdscript_rendering.md) resolution different from the one specified in size, by calling:
...
For 3D, a [Viewport](../godot_gdscript_rendering.md) will contain a [World3D](../godot_gdscript_misc.md). This is basically the universe that links physics and rendering together. Node3D-based nodes will register using the World3D of the closest Viewport. By default, newly created Viewports do not contain a World3D but use the same as their parent Viewport. The Root Viewport always contains a World3D, which is the one objects are rendered to by default.

A [World3D](../godot_gdscript_misc.md) can be set in a [Viewport](../godot_gdscript_rendering.md) using the [World 3D](../godot_gdscript_misc.md) property, that will separate all children nodes of this [Viewport](../godot_gdscript_rendering.md) and will prevent them from interacting with the parent Viewport's World3D. This is especially useful in scenarios where, for example, you might want to show a separate character in 3D imposed over the game (like in StarCraft).

As a helper for situations where you want to create [Viewports](../godot_gdscript_rendering.md) that display single objects and don't want to create a [World3D](../godot_gdscript_misc.md), Viewport has the option to use its [Own World3D](../godot_gdscript_misc.md). This is useful when you want to instance 3D characters or objects in [World2D](../godot_gdscript_misc.md).

For 2D, each [Viewport](../godot_gdscript_rendering.md) always contains its own [World2D](../godot_gdscript_misc.md). This suffices in most cases, but in case sharing them may be desired, it is possible to do so by setting [world_2d](../godot_gdscript_misc.md) on the Viewport through code.
...
It is possible to query a capture of the [Viewport](../godot_gdscript_rendering.md) contents. For the Root Viewport, this is effectively a screen capture. This is done with the following code:
...
But if you use this in `_ready()` or from the first frame of the [Viewport's](../godot_gdscript_rendering.md) initialization, you will get an empty texture because there is nothing to get as texture. You can deal with it using (for example):
...
If the [SubViewport](../godot_gdscript_rendering.md) is a child of a [SubViewportContainer](../godot_gdscript_ui_controls.md), it will become active and display anything it has inside. The layout looks like this:

The [SubViewport](../godot_gdscript_rendering.md) will cover the area of its parent [SubViewportContainer](../godot_gdscript_ui_controls.md) completely if [Stretch](../godot_gdscript_misc.md) is set to `true` in the SubViewportContainer.

> **Note:** The size of the [SubViewportContainer](../godot_gdscript_ui_controls.md) cannot be smaller than the size of the [SubViewport](../godot_gdscript_rendering.md).

### Rendering

Due to the fact that the [Viewport](../godot_gdscript_rendering.md) is an entryway into another rendering surface, it exposes a few rendering properties that can be different from the project settings. You can choose to use a different level of [MSAA](../godot_gdscript_misc.md) for each Viewport. The default behavior is `Disabled`.

If you know that the [Viewport](../godot_gdscript_rendering.md) is only going to be used for 2D, you can [Disable 3D](../godot_gdscript_misc.md). Godot will then restrict how the Viewport is drawn. Disabling 3D is slightly faster and uses less memory compared to enabled 3D. It's a good idea to disable 3D if your viewport doesn't render anything in 3D.

> **Note:** If you need to render 3D shadows in the viewport, make sure to set the viewport's [positional_shadow_atlas_size](../godot_gdscript_misc.md) property to a value higher than `0`. Otherwise, shadows won't be rendered. By default, the equivalent project setting is set to `4096` on desktop platforms and `2048` on mobile platforms.

Godot also provides a way of customizing how everything is drawn inside [Viewports](../godot_gdscript_rendering.md) using [Debug Draw](../godot_gdscript_misc.md). Debug Draw allows you to specify a mode which determines how the Viewport will display things drawn inside it. Debug Draw is `Disabled` by default. Some other options are `Unshaded`, `Overdraw`, and `Wireframe`. For a full list, refer to the [Viewport Documentation](../godot_gdscript_rendering.md).
...
When rendering to a [SubViewport](../godot_gdscript_rendering.md), whatever is inside will not be visible in the scene editor. To display the contents, you have to draw the SubViewport's [ViewportTexture](../godot_gdscript_rendering.md) somewhere. This can be requested via code using (for example):
...
and then selecting the [Viewport](../godot_gdscript_rendering.md) you want to use.

Every frame, the [Viewport's](../godot_gdscript_rendering.md) texture is cleared away with the default clear color (or a transparent color if [Transparent BG](../godot_gdscript_misc.md) is set to `true`). This can be changed by setting [Clear Mode](../godot_gdscript_misc.md) to `Never` or `Next Frame`. As the name implies, Never means the texture will never be cleared, while next frame will clear the texture on the next frame and then set itself to Never.

By default, re-rendering of the [SubViewport](../godot_gdscript_rendering.md) happens when its [ViewportTexture](../godot_gdscript_rendering.md) has been drawn in a frame. If visible, it will be rendered, otherwise, it will not. This behavior can be changed by setting [Update Mode](../godot_gdscript_misc.md) to `Never`, `Once`, `Always`, or `When Parent Visible`. Never and Always will never or always re-render respectively. Once will re-render the next frame and change to Never afterwards. This can be used to manually update the Viewport. This flexibility allows users to render an image once and then use the texture without incurring the cost of rendering every frame.
