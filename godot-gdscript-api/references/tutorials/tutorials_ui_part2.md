# Godot 4 GDScript Tutorials — Ui (Part 2)
...
Additional windows can be created using the [Window](../godot_gdscript_ui_controls.md) node. Windows can be moved, resized, minimized, and closed independently from the main application window.

However, if you close the main window, all other windows will also be closed since closing the main window ends the process. You can avoid this by minimizing the main window, setting its [unfocusable](../godot_gdscript_misc.md) property to `true` (to hide it from the taskbar and task switcher), then creating additional Window nodes immediately on startup. Remember to provide an alternative means of exiting the application in this case, such as a **tray icon**.
...
Size limits can be enforced using the [min_size](../godot_gdscript_misc.md) and [max_size](../godot_gdscript_misc.md) properties on a Window node. Remember to multiply these size limits according to the application's scale factor (see **Scaling to hiDPI displays** for details).

> **Tip:** As a reminder, you can retrieve the root Window node to set properties on it using [get_window()](../godot_gdscript_misc.md) on any Node.

#### Using native file dialogs

_This is only supported on Windows, macOS, Linux, and Android._

By default, Godot uses its own [FileDialog](../godot_gdscript_ui_controls.md) implementation for file dialogs. However, you can use the operating system's native file dialogs instead. This is generally preferred by users, as native file dialogs better integrate with the desktop environment and provide a more familiar experience.

You can opt into native file dialogs by enabling the [use_native_dialog](../godot_gdscript_misc.md) property on the FileDialog node. This must be done on each FileDialog node used in the project, as there is no project setting to control this behavior globally.

> **Note:** See the [property description](../godot_gdscript_misc.md) for details on platform support. Additionally, on macOS, native file dialogs are not supported when game embedding is enabled in the editor. To test this functionality when running the project, make sure you disable game embedding by switching to the Game screen, clicking the 3 vertical dots and unchecking Embed Game on Next Play.

#### Creating an icon in the system tray

_This is only supported on Windows and macOS._

You can create one or more icons in the system tray (also known as the notification area) using a [StatusIndicator](../godot_gdscript_misc.md) node. In addition to a tooltip, this node can have a [PopupMenu](../godot_gdscript_ui_controls.md) node assigned, so that a dropdown can be shown when clicking the icon.

StatusIndicator also has a [pressed](../godot_gdscript_misc.md) signal that is emitted when the icon is clicked. Use this to perform an action without making a dropdown show up, or perform different actions depending on which mouse button was pressed.
    ...
    if tray_icon_supported:
        ...
        pressed.connect(
                func(_mouse_button, _position):
                    # Restore the application when the tray icon is
# ...
```
...
For more advanced use cases, you can also use the [NativeMenu](../godot_gdscript_misc.md) singleton directly without using the MenuBar node.
...
Godot currently only supports client-side decorations on macOS. This can be used by enabling the [display/window/size/extend_to_title](../godot_gdscript_misc.md) project setting.
...
To conditionally adapt your UI according to whether client-side decorations are enabled, use [DisplayServer.has_feature](../godot_gdscript_misc.md) and also check the current value of [Window.extend_to_title](../godot_gdscript_ui_controls.md) (which is what the project setting changes):
...
To correctly position the window title, consider using [DisplayServer.window_get_safe_title_margins()](../godot_gdscript_misc.md) which returns a Vector3 where `x` is the left margin, `y` is the right margin (will increase when the system uses right-to-left typesetting), and `z` is the height. Additionally, you can call [DisplayServer.window_set_window_buttons_offset()](../godot_gdscript_misc.md) to adjust the position of the close/minimize/maximize buttons (usually to vertically center them).
...
func _ready():
    send_notification("Success", "Operation co
# ...
```
...
- [application/boot_splash/bg_color](../godot_gdscript_misc.md) set to a transparent black color (RGBA: 0, 0, 0, 0).
- [application/boot_splash/show_image](../godot_gdscript_misc.md) disabled.
- [display/window/size/borderless](../godot_gdscript_misc.md) enabled.
- [display/window/size/no_focus](../godot_gdscript_misc.md) enabled.
- [display/window/size/window_width_override](../godot_gdscript_misc.md) set to `1`.
- [display/window/size/window_height_override](../godot_gdscript_misc.md) set to `1`.
- [display/window/per_pixel_transparency/allowed](../godot_gdscript_misc.md) enabled.
- [display/window/size/transparent](../godot_gdscript_misc.md) enabled.
- [rendering/viewport/transparent_background](../godot_gdscript_misc.md) enabled.
...
- [display/window/size/borderless](../godot_gdscript_misc.md)
- [display/window/per_pixel_transparency/allowed](../godot_gdscript_misc.md)
- [display/window/size/transparent](../godot_gdscript_misc.md)
- [rendering/viewport/transparent_background](../godot_gdscript_misc.md)
- [display/window/size/always_on_top](../godot_gdscript_misc.md)
- [display/window/size/no_focus](../godot_gdscript_misc.md)
...
To allow mouse input to pass through to the background application, set the [mouse_passthrough](../godot_gdscript_misc.md) property to `true` on the Window that is being drawn as an overlay. You can also define a polygon in [mouse_passthrough_polygon](../godot_gdscript_misc.md), so that certain areas can still intercept mouse input on the overlay.

Additionally, you may want to set the [exclude_from_capture](../godot_gdscript_misc.md) property to `true` to prevent the overlay from appearing in screenshots or recordings. This hint is implemented on Windows and macOS only, and it is on a best-effort basis, so it should not be used as an absolute security measure or DRM.
...
Godot automatically enables screen reader support if a screen reader is detected as running. This can be configured in the Project Settings using [accessibility/general/accessibility_support](../godot_gdscript_misc.md) to disable it in situations where it is not desired. It can also be forcibly enabled, which is useful when using accessibility debugging tools that are not recognized as screen readers by Godot.
...
To get screen reader support to a good level of usability, significant amounts of work are required. You need to define accessibility labels using the [Control.accessibility_name](../godot_gdscript_ui_controls.md) and [Control.accessibility_description](../godot_gdscript_ui_controls.md) properties, and ensure the UI flows in a logical order when read by a screen reader.
...
- Enable [application/config/use_custom_user_dir](../godot_gdscript_misc.md) and set [application/config/custom_user_dir_name](../godot_gdscript_misc.md) to a suitable name for your application. This ensures user settings and files are stored in a dedicated folder instead of the [default Godot folder](tutorials_io.md). By convention, it's a good idea to use normal case on Windows (e.g. `Application Name`) and kebab-case (e.g. `application-name`) on macOS and Linux.
- Configure native icons that match the operating system's design guidelines using [application/config/windows_native_icon](../godot_gdscript_misc.md) (in ICO format) and [application/config/macos_native_icon](../godot_gdscript_misc.md) (in ICNS format). By default, Godot will automatically generate native icons based on the project icon, but this is not always optimal.
...
- Disable [display/window/subwindows/embed_subwindows](../godot_gdscript_misc.md), so that additional windows use the operating system theming and are seen as native operating system windows.
...
- Enable [application/run/low_processor_mode](../godot_gdscript_misc.md) to decrease CPU and GPU usage. This makes the project only render a frame if something on screen has changed.

- Note that in certain cases, the project has to redraw continuously (e.g. if an animation or shader using `TIME` is visible). This will result in significant power draw if done for a long time, which leads to reduced battery life and increased fan noise. To troubleshoot situations where the project redraws continuously, you can enable Debug > Debug Canvas Item Redraws at the top of the editor, then run the project. Areas that are redrawn will be highlighted in red for a second. The highlighting color and duration can be adjusted using the [debug/canvas_items/debug_redraw_time](../godot_gdscript_misc.md) and [debug/canvas_items/debug_redraw_color](../godot_gdscript_misc.md) project settings.
- The maximum framerate at which the application can draw is determined by [application/run/low_processor_mode_sleep_usec](../godot_gdscript_misc.md). This value is expressed in microseconds per frame, so the maximum FPS can be obtained using the formula `1000000.0 / sleep_usec`. By default, this is set to `6900`, which corresponds to a maximum of approximately 145 FPS. You can increase this value to further reduce CPU and GPU usage, at the cost of a less smooth experience.
- Disable [display/window/energy_saving/keep_screen_on](../godot_gdscript_misc.md), so that the screen can turn off according to the operating system's power settings when the application is idle. This behavior is normally not desired in a game (e.g. when watching cutscenes), but in applications, we want the screen to turn off to save power when the user is not actively using the application.
- Set [audio/driver/driver](../godot_gdscript_misc.md) to `Dummy` _(case-sensitive)_ if your application does not require audio output or input. This prevents the audio server from starting, which saves some CPU and memory resources. This also prevents the application from showing up in the list of applications playing audio in the operating system's audio mixer. On macOS, this also ensures the application does not prevent the device from sleeping.
- Set [physics/2d/physics_engine](../godot_gdscript_misc.md) and [physics/3d/physics_engine](../godot_gdscript_misc.md) to `Dummy` if your application does not require physics simulation (including object picking). This prevents the physics servers from starting, which saves CPU and memory resources. This also allows the [engine compilation configuration editor](tutorials_editor.md) to automatically detect the fact that the project doesn't use physics.
- Consider setting [display/window/vsync/vsync_mode](../godot_gdscript_misc.md) to **Disabled** to reduce input lag. This is particularly helpful in latency-sensitive projects such as drawing applications. This may increase power usage and cause screen tearing, so it's recommended to provide an option for the user to toggle V-Sync as needed.
...
- Enable [input_devices/pointing/android/enable_long_press_as_right_click](../godot_gdscript_misc.md) to allow users to perform right-click actions using a long press gesture.
- Enable [input_devices/pointing/android/enable_pan_and_scale_gestures](../godot_gdscript_misc.md) to allow users to pan and zoom using touch gestures. This will emulate [InputEventPanGesture](../godot_gdscript_input.md) and [InputEventMagnifyGesture](../godot_gdscript_input.md) events, which can be handled in your project's code and are normally emitted by laptop trackpads.
- Disable Screen > Immersive Mode in the Android export preset to show the system status and navigation bars while the application is active. Additionally, enable Screen > Edge to Edge to make status bars and navigation icons translucent and draw on top of the application. If you do so, make sure your application leaves enough space available for the status bar and navigation icons. You can use [DisplayServer.get_display_safe_area](../godot_gdscript_misc.md) and [DisplayServer.get_display_cutouts](../godot_gdscript_misc.md) to query the area in which your application can safely draw.

**iOS:**

- Disable [display/window/ios/hide_home_indicator](../godot_gdscript_misc.md) to show the home indicator on top of the application.
- Disable [display/window/ios/hide_status_bar](../godot_gdscript_misc.md) to keep the status bar visible when the application is active.
- Disable [display/window/ios/suppress_ui_gesture](../godot_gdscript_misc.md) to allow UI gestures to work immediately, without requiring them to be done twice.
...
- [ConfigFile](../godot_gdscript_filesystem.md) (used to save user preferences)
...
Unlike 2D nodes, "size" is important with controls, as it helps to organize them in proper layouts. For this, the [Control.size](../godot_gdscript_ui_controls.md) property is provided. Checking it during `_draw()` is vital to ensure everything is kept in-bounds.

#### Checking focus

Some controls (such as buttons or text editors) might provide input focus for keyboard or joypad input. Examples of this are entering text or pressing a button. This is controlled with the [Control.focus_mode](../godot_gdscript_ui_controls.md) property. When drawing, and if the control supports input focus, it is always desired to show some sort of indicator (highlight, box, etc.) to indicate that this is the currently focused control. To check for this status, the [Control.has_focus()](../godot_gdscript_ui_controls.md) method exists. Example
...
As mentioned before, size is important to controls. This allows them to lay out properly, when set into grids, containers, or anchored. Controls, most of the time, provide a _minimum size_ to help properly lay them out. For example, if controls are placed vertically on top of each other using a [VBoxContainer](../godot_gdscript_ui_controls.md), the minimum size will make sure your custom control is not squished by the other controls in the container.

To provide this callback, just override [Control.\_get_minimum_size()](../godot_gdscript_ui_controls.md), for example:
...
- Control provides keyboard/joypad focus via [Control.focus_mode](../godot_gdscript_ui_controls.md).

This function is [Control.\_gui_input()](../godot_gdscript_ui_controls.md). To use it, override it in your control. No processing needs to be set.
...
All these situations require a more capable OS-like user interface, with advanced layout and formatting. For that, [Containers](../godot_gdscript_ui_controls.md) are more useful.
...
When a [Container](../godot_gdscript_ui_controls.md)-derived node is used, all children [Control](../godot_gdscript_ui_controls.md) nodes give up their own positioning ability. This means the _Container_ will control their positioning and any attempt to manually alter these nodes will be either ignored or invalidated the next time their parent is resized.
...
Arranges child controls vertically or horizontally (via [HBoxContainer](../godot_gdscript_ui_controls.md) and [VBoxContainer](../godot_gdscript_ui_controls.md)). In the opposite of the designated direction (as in, vertical for a horizontal container), it just expands the children.

These containers make use of the _Stretch Ratio_ property for children with the _Expand_ flag set.

#### Grid Container

Arranges child controls in a grid layout (via [GridContainer](../godot_gdscript_ui_controls.md), amount of columns must be specified). Uses both the vertical and horizontal expand flags.

#### Margin Container

Child controls are expanded towards the bounds of this control (via [MarginContainer](../godot_gdscript_ui_controls.md)). Padding will be added on the margins depending on the theme configuration.
...
Allows you to place several child controls stacked on top of each other (via [TabContainer](../godot_gdscript_ui_controls.md)), with only the _current_ one visible.
...
Accepts only one or two children controls, then places them side to side with a divisor (via [HSplitContainer](../godot_gdscript_ui_controls.md) and [VSplitContainer](../godot_gdscript_ui_controls.md)). Respects both horizontal and vertical flags, as well as _Ratio_.

The divisor can be dragged around to change the size relation between both children:

#### PanelContainer

A container that draws a _StyleBox_, then expands children to cover its whole area (via [PanelContainer](../godot_gdscript_ui_controls.md), respecting the _StyleBox_ margins). It respects both the horizontal and vertical sizing options.

This container is useful as a top-level control, or just to add custom backgrounds to sections of a layout.

#### FoldableContainer

A container that can be expanded/collapsed (via [FoldableContainer](../godot_gdscript_misc.md)). Child controls are hidden when it is collapsed.

#### ScrollContainer

Accepts a single child node. If the child node is bigger than the container, scrollbars will be added to allow panning the node around (via [ScrollContainer](../godot_gdscript_ui_controls.md)). Both vertical and horizontal size options are respected, and the behavior can be turned on or off per axis in the properties.
...
A container type that arranges its child controls in a way that preserves their proportions automatically when the container is resized. (via [AspectRatioContainer](../godot_gdscript_ui_controls.md)). It has multiple stretch modes, providing options for adjusting the child controls' sizes concerning the container: "fill," "width control height," "height control width," and "cover."
...
FlowContainer is a container that arranges its child controls either horizontally or vertically (via [HFlowContainer](../godot_gdscript_misc.md) and via [VFlowContainer](../godot_gdscript_misc.md)). When the available space runs out, it wraps the children to the next line or column, similar to how text wraps in a book.
...
The mode setting defines how a node can be focused. **All** means a node can be focused by clicking on it with the mouse, or selecting it with a keyboard or controller. **Click** means it can only be focused on by clicking on it. Finally, **None** means it can't be focused at all. Different control nodes have different default settings for this based on how they are typically used, for example, [Label](../godot_gdscript_ui_controls.md) nodes are set to "None" by default, while [buttons](../godot_gdscript_misc.md) are set to "All".
...
You can use the [Control.grab_focus()](../godot_gdscript_ui_controls.md) method to focus a control. Here is a basic example of setting initial focus with code:
