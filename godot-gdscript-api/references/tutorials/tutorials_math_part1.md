# Godot 4 GDScript Tutorials — Math (Part 1)
...
There are two objects that contain curves: [Curve3D](../godot_gdscript_resources.md) and [Curve2D](../godot_gdscript_resources.md) (for 3D and 2D respectively).

They can contain several points, allowing for longer paths. It is also possible to set them to nodes: [Path3D](../godot_gdscript_nodes_3d.md) and [Path2D](../godot_gdscript_nodes_2d.md) (also for 3D and 2D respectively):
...
The _Curve_ classes provide this via the [Curve2D.tessellate()](../godot_gdscript_resources.md) function (which receives optional `stages` of recursion and angle `tolerance` arguments). This way, drawing something based on a curve is easier.
...
Vector types ([Vector2](../godot_gdscript_math_types.md) and [Vector3](../godot_gdscript_math_types.md)) can also be interpolated, they come with handy functions to do it [Vector2.lerp()](../godot_gdscript_math_types.md) and [Vector3.lerp()](../godot_gdscript_math_types.md).

For cubic interpolation, there are also [Vector2.cubic_interpolate()](../godot_gdscript_math_types.md) and [Vector3.cubic_interpolate()](../godot_gdscript_math_types.md), which do a Bezier style interpolation.
...
It is also possible to interpolate whole transforms (make sure they have either uniform scale or, at least, the same non-uniform scale). For this, the function [Transform3D.interpolate_with()](../godot_gdscript_math_types.md) can be used.
...
Most of this guide focuses on 2D, using [Transform2D](../godot_gdscript_math_types.md) and [Vector2](../godot_gdscript_math_types.md), but the way things work in 3D is very similar.
...
You might have noticed that [Transform2D](../godot_gdscript_math_types.md) actually has three [Vector2](../godot_gdscript_math_types.md) values: `x`, `y`, and `origin`. The `origin` value is not part of the basis, but it is part of the transform, and we need it to represent position. From now on we'll keep track of the origin vector in all examples. You can think of origin as another column, but it's often better to think of it as completely separate.

Note that in 3D, Godot has a separate [Basis](../godot_gdscript_math_types.md) structure for holding the three [Vector3](../godot_gdscript_math_types.md) values of the basis, since the code can get complex and it makes sense to separate it from [Transform3D](../godot_gdscript_math_types.md) (which is composed of one [Basis](../godot_gdscript_math_types.md) and one extra [Vector3](../godot_gdscript_math_types.md) for the origin).
...
You may have noticed that a transform has more degrees of freedom than the combination of the above actions. The basis of a 2D transformation matrix has four total numbers in two [Vector2](../godot_gdscript_math_types.md) values, while a rotation value and a Vector2 for scale only has 3 numbers. The high-level concept for the missing degree of freedom is called _shearing_.
...
In actual projects, you will usually be working with transforms inside transforms by having multiple [Node2D](../godot_gdscript_nodes_2d.md) or [Node3D](../godot_gdscript_nodes_3d.md) nodes parented to each other.

However, it's useful to understand how to manually calculate the values we need. We will go over how you could use [Transform2D](../godot_gdscript_math_types.md) or [Transform3D](../godot_gdscript_math_types.md) to manually calculate transforms of nodes.
...
One of the great things about transformation matrices is that they work very similarly between 2D and 3D transformations. All the code and formulas used above for 2D work the same in 3D, with 3 exceptions: the addition of a third axis, that each axis is of type [Vector3](../godot_gdscript_math_types.md), and also that Godot stores the [Basis](../godot_gdscript_math_types.md) separately from the [Transform3D](../godot_gdscript_math_types.md), since the math can get complex and it makes sense to separate it.
...
In 3D we do not typically use angles, we either use a transformation basis (used pretty much everywhere in Godot), or we use quaternions. Godot can represent quaternions using the [Quaternion](../godot_gdscript_math_types.md) struct. My suggestion to you is to completely ignore how they work under-the-hood, because they are very complicated and unintuitive.
...
Godot exposes two ways to generate random numbers: via _global scope_ methods or using the [RandomNumberGenerator](../godot_gdscript_misc.md) class.
...
We can use random integer generation to get a random element from an array, or use the [Array.pick_random](../godot_gdscript_misc.md) method to do it for us:
...
To prevent the same fruit from being picked more than once in a row, we can add more logic to the above method. In this case, we can't use [Array.pick_random](../godot_gdscript_misc.md) since it lacks a way to prevent repetition:
...
You can also get a weighted random _index_ using the [rand_weighted()](../godot_gdscript_misc.md) method on a RandomNumberGenerator instance. This returns a random integer between 0 and the size of the array that is passed as a parameter. Each value in the array is a floating-point number that represents the _relative_ likelihood that it will be returned as an index. A higher value means the value is more likely to be returned as an index, while a value of `0` means it will never be returned as an index.
...
To achieve this, you can use random _noise_ functions. Noise functions are especially popular in procedural generation to generate realistic-looking terrain. Godot provides [FastNoiseLite](../godot_gdscript_resources.md) for this, which supports 1D, 2D and 3D noise. Here's an example with 1D noise:
...
Godot offers a [Crypto](../godot_gdscript_networking.md) class for this. This class can perform asymmetric key encryption/decryption, signing/verification, while also generating cryptographically secure random bytes, RSA keys, HMAC digests, and self-signed [X509Certificate](../godot_gdscript_networking.md)s.
...
> **See also:** See [PackedByteArray](../godot_gdscript_misc.md)'s documentation for other methods you can use to decode the generated bytes into various types of data, such as integers or floats.
