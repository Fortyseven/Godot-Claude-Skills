# Godot 4 GDScript Tutorials — I18N (Part 1)
...
It is recommended to default to the user's preferred language which can be obtained via [OS.get_locale_language()](../godot_gdscript_misc.md). If your game is not available in that language, it will fall back to the [Fallback](../godot_gdscript_misc.md) in Project > Project Settings > General > Internationalization > Locale, or to `en` if empty. Nevertheless, letting players change the language in game is recommended for various reasons (e.g. translation quality or player preference).
...
Some controls, such as [Button](../godot_gdscript_ui_controls.md) and [Label](../godot_gdscript_ui_controls.md), will automatically fetch a translation if their text matches a translation key. For example, if a label's text is `MAIN_SCREEN_GREETING1` and that key exists in the current translation, then the text will automatically be translated.
...
In code, the [Object.tr()](../godot_gdscript_core.md) function can be used. This will just look up the text in the translations and convert it if found:
...
The same text in different languages can vary greatly in length. For this, make sure to read the tutorial on [Size and anchors](tutorials_ui.md), as dynamically adjusting control sizes may help. [Container](../godot_gdscript_ui_controls.md) can be useful, as well as the text wrapping options available in [Label](../godot_gdscript_ui_controls.md).
...
Godot has a server handling low-level translation management called the [TranslationServer](../godot_gdscript_misc.md). Translations can be added or removed during runtime; the current language can also be changed at runtime.
...
Controls specifically designed for number input or output (e.g. ProgressBar, SpinBox) will use localized numbering system automatically, for the other control [TextServer.format_number(string, language)](../godot_gdscript_misc.md) can be used to convert Western Arabic numbers (0..9) to the localized numbering system and [TextServer.parse_number(string, language)](../godot_gdscript_misc.md) to convert it back.
...
> **Note:** Automatic Control translation is not supported when using plural forms. You must translate the string manually using [tr_n()](../godot_gdscript_misc.md).
...
> **Note:** Automatic Control translation is not supported when using context. You must translate the string manually using [tr()](../godot_gdscript_misc.md) or [tr_n()](../godot_gdscript_misc.md).
