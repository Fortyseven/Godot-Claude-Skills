# Godot 4 GDScript Tutorials — Shaders (Part 1)
...
To get around the limitation on using the depth buffer, use a [MeshInstance3D](../godot_gdscript_nodes_3d.md) with a [QuadMesh](../godot_gdscript_rendering.md) primitive. This allows us to use a shader and to access the depth texture of the scene. Next, use a vertex shader to make the quad cover the screen at all times so that the post-processing effect will be applied at all times, including in the editor.
...
Set the Mesh in the MeshInstance3D to an [ArrayMesh](../godot_gdscript_rendering.md). An ArrayMesh is a tool that allows you to easily construct a Mesh from Arrays for vertices, normals, colors, etc.
...
Now to execute our shader we need a local [RenderingDevice](../godot_gdscript_rendering.md) which can be created using the [RenderingServer](../godot_gdscript_rendering.md):
...
The `buffer`, `pipeline`, and `uniform_set` variables we've been using are each an [RID](../godot_gdscript_math_types.md). Because RenderingDevice is meant to be a lower-level API, RIDs aren't freed automatically. This means that once you're done using `buffer` or any other RID, you are responsible for freeing its memory manually using the RenderingDevice's [free_rid()](../godot_gdscript_misc.md) method.
...
Post-processing effects are shaders applied to a frame after Godot has rendered it. To apply a shader to a frame, create a [CanvasLayer](../godot_gdscript_nodes_2d.md), and give it a [ColorRect](../godot_gdscript_ui_controls.md). Assign a new [ShaderMaterial](../godot_gdscript_rendering.md) to the newly created `ColorRect`, and set the `ColorRect`'s anchor preset to Full Rect:

Your scene tree will look something like this:

> **Note:** Another more efficient method is to use a [BackBufferCopy](../godot_gdscript_misc.md) to copy a region of the screen to a buffer and to access it in a shader script through a `sampler2D` using `hint_screen_texture`.
...
For this demo, we will use this [Sprite](../godot_gdscript_misc.md) of a sheep.

Assign a new [Shader](../godot_gdscript_rendering.md) to the `ColorRect`'s `ShaderMaterial`. You can access the frame's texture and UV with a `sampler2D` using `hint_screen_texture` and the built-in `SCREEN_UV` uniforms.
...
7. The `fog()` function runs for every froxel in the volumetric fog froxel buffer that intersects with the [FogVolume](../godot_gdscript_nodes_3d.md). Used by fog shaders.
...
- sky to render [Skies](../godot_gdscript_misc.md).
- fog to render [FogVolumes](../godot_gdscript_nodes_3d.md)
...
In 2D, this can be corrected via the [BackBufferCopy](../godot_gdscript_misc.md) node, which can be instantiated between both spheres. BackBufferCopy can work by either specifying a screen region or the whole screen:
...
You can reproduce the back-buffer logic in 3D by creating a [Viewport](../godot_gdscript_rendering.md) with a camera in the same position as your object, and then use the [Viewport's](../godot_gdscript_rendering.md) texture instead of the screen texture.
