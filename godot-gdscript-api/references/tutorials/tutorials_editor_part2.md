# Godot 4 GDScript Tutorials — Editor (Part 2)
...
There are dozens of settings you can change to control a project's execution, including physics, rendering, and windowing settings. These settings can be changed from the **Project Settings** window, from code, or by manually editing the `project.godot` file. You can see a full list of settings in the [ProjectSettings](../godot_gdscript_filesystem.md) class.
...
You can use [set_setting()](../godot_gdscript_misc.md) to change a setting's value from code:
...
However, many project settings are only read once when the game starts. After that, changing the setting with `set_setting()` will have no effect. Instead, most settings have a corresponding property or method on a runtime class like [Engine](../godot_gdscript_core.md) or [DisplayServer](../godot_gdscript_misc.md):
...
In general, project settings are duplicated at runtime in the [Engine](../godot_gdscript_core.md), [PhysicsServer2D](../godot_gdscript_physics.md), [PhysicsServer3D](../godot_gdscript_physics.md), [RenderingServer](../godot_gdscript_rendering.md), [Viewport](../godot_gdscript_rendering.md), or [Window](../godot_gdscript_ui_controls.md) classes. In the [ProjectSettings](../godot_gdscript_filesystem.md) class reference, settings links to their equivalent runtime property or method.

### Reading project settings

You can read project settings with [get_setting()](../godot_gdscript_misc.md) or [get_setting_with_override()](../godot_gdscript_misc.md):
...
- **Run**: If the script extends [EditorScript](../godot_gdscript_editor.md) and intended to be executed without running the project, this option runs the script. See [Running one-off scripts using EditorScript](tutorials_plugins.md) for more.
