# Godot 4 GDScript Tutorials — Ui (Part 3)

> 3 tutorials. GDScript-specific code examples.

## Introduction to GUI skinning

It is essential for a game to provide clear, informative, and yet visually pleasing user interface to its players. While [Control](../godot_gdscript_ui_controls.md) nodes come with a decently functional look out of the box, there is always room for uniqueness and case-specific tuning. For this purpose Godot engine includes a system for GUI skinning (or theming), which allows you to customize the look of every control in your user interface, including your custom controls.
...
A [color](../godot_gdscript_misc.md) value, which is often used for fonts and backgrounds. Colors can also be used for modulation of controls and icons.

- **Constant**

An integer value, which can be used either for numeric properties of controls (such as the item separation in a [BoxContainer](../godot_gdscript_ui_controls.md)), or for boolean flags (such as the drawing of relationship lines in a [Tree](../godot_gdscript_ui_controls.md)).

- **Font**

A [font](../godot_gdscript_misc.md) resource, which is used by controls that display text. Fonts contain most text rendering settings, except for its size and color. On top of that, alignment and text direction are controlled by individual controls.
...
A [texture](../godot_gdscript_misc.md) resource, which is normally used to display an icon (on a [Button](../godot_gdscript_ui_controls.md), for example).

- **StyleBox**

A [StyleBox](../godot_gdscript_resources.md) resource, a collection of configuration options which define the way a UI panel should be displayed. This is not limited to the [Panel](../godot_gdscript_ui_controls.md) control, as styleboxes are used by many controls for their backgrounds and overlays.
...
Local overrides are less useful for the visual flair of your user interface, especially if you aim for consistency. However, for layout nodes these are essential. Nodes such as [BoxContainer](../godot_gdscript_ui_controls.md) and [GridContainer](../godot_gdscript_ui_controls.md) use theme constants for defining separation between their children, and [MarginContainer](../godot_gdscript_ui_controls.md) stores its customizable margins in its theme items.
...
When designing a user interface there may be times when a [Control](../godot_gdscript_ui_controls.md) node needs to have a different look than what is normally defined by a [Theme](../godot_gdscript_resources.md). Every control node has theme property overrides, which allow you to redefine the styling for each individual UI element.

This approach quickly becomes hard to manage, if you need to share the same custom look between several controls. Imagine that you use gray, blue, and red variants of [Button](../godot_gdscript_ui_controls.md) throughout your project. Setting it up every time you add a new button element to your interface is a tedious task.
...
Font outlines are available in most nodes that derive from Control, in addition to [Label3D](../godot_gdscript_misc.md).
...
Support for font shadows is more limited: they are only available in [Label](../godot_gdscript_ui_controls.md) and [RichTextLabel](../godot_gdscript_ui_controls.md). Additionally, font shadows always have a hard edge (but you can reduce their opacity to make them look more subtle). To enable font shadows on a given node, configure the **Font Shadow Color**, **Shadow Offset X**, and **Shadow Offset Y** theme overrides in a Label or RichTextLabel node accordingly:

The result should look like this:

> **Tip:** You can create local overrides to font display in Label nodes by creating a [LabelSettings](../godot_gdscript_misc.md) resource that you reuse across Label nodes. This resource takes priority over theme properties.
...
To use a variable font, create a [FontVariation](../godot_gdscript_resources.md) resource in the location where you intend to use the font, then load a font file within the FontVariation resource:
...
Faux bold and italic is automatically used in [RichTextLabel](../godot_gdscript_ui_controls.md)'s bold and italic tags if no custom fonts are provided for bold and/or italic.
...
- **Top:** Additional spacing above glyphs. This is used for multiline text, but also to calculate the minimum size of controls such as [Label](../godot_gdscript_ui_controls.md) and [Button](../godot_gdscript_ui_controls.md).
- **Bottom:** Additional spacing below glyphs. This is used for multiline text, but also to calculate the minimum size of controls such as [Label](../godot_gdscript_ui_controls.md) and [Button](../godot_gdscript_ui_controls.md).
...
Create a [SystemFont](../godot_gdscript_resources.md) resource in the location where you desire to use the system font:
