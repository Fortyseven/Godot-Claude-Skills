...
[CanvasItem](../godot_csharp_nodes_2d.md) is the base for all 2D nodes, be it regular 2D nodes, such as [Node2D](../godot_csharp_nodes_2d.md), or [Control](../godot_csharp_ui_controls.md). Both inherit from [CanvasItem](../godot_csharp_nodes_2d.md). You can arrange canvas items in trees. Each item will inherit its parent's transform: when the parent moves, its children move too.

CanvasItem nodes, and nodes inheriting from them, are direct or indirect children of a [Viewport](../godot_csharp_rendering.md), that displays them.

The Viewport's property [Viewport.canvas_transform](../godot_csharp_rendering.md), allows to apply a custom [Transform2D](../godot_csharp_math_types.md) transform to the CanvasItem hierarchy it contains. Nodes such as [Camera2D](../godot_csharp_nodes_2d.md) work by changing that transform.
...
Add a script to any [CanvasItem](../godot_csharp_nodes_2d.md) derived node, like [Control](../godot_csharp_ui_controls.md) or [Node2D](../godot_csharp_nodes_2d.md). Then override the [\_draw()](../godot_csharp_misc.md) function.
...
Draw commands are described in the [CanvasItem](../godot_csharp_nodes_2d.md) class reference. There are plenty of them and we will see some of them in the examples below.

### Updating

The [\_draw](../godot_csharp_misc.md) function is only called once, and then the draw commands are cached and remembered, so further calls are unnecessary.

If re-drawing is required because a variable or something else changed, call [CanvasItem.queue_redraw](../godot_csharp_nodes_2d.md) in that same node and a new `_draw()` call will happen.
...
In some cases, we may need to redraw every frame. For this, call [queue_redraw](../godot_csharp_misc.md) from the [\_process](../godot_csharp_misc.md) method, like this:
...
The drawing API uses the CanvasItem's coordinate system, not necessarily pixel coordinates. This means `_draw()` uses the coordinate space created after applying the CanvasItem's transform. Additionally, you can apply a custom transform on top of it by using [draw_set_transform](../godot_csharp_misc.md) or [draw_set_transform_matrix](../godot_csharp_misc.md).

When using [draw_line](../godot_csharp_misc.md), you should consider the width of the line. When using a width that is an odd size, the position of the start and end points should be shifted by `0.5` to keep the line centered, as shown below.
...
The same applies to the [draw_rect](../godot_csharp_misc.md) method with `filled = false`.
...
Godot offers method parameters in [draw_line](../godot_csharp_misc.md) to enable antialiasing, but not all custom drawing methods offer this `antialiased` parameter.
...
> **Note:** The following instructions use a fixed set of coordinates that could be too small for high resolution screens (larger than 1080p). If that is your case, and the drawing is too small consider increasing your window scale in the project setting [Display > Window > Stretch > Scale](../godot_csharp_misc.md) to adjust the project to a higher resolution (a 2 or 4 scale tends to work well).

#### Drawing a custom polygon shape

While there is a dedicated node to draw custom polygons ( [Polygon2D](../godot_csharp_nodes_2d.md)), we will use in this case exclusively lower level drawing functions to combine them on the same node and be able to create more complex shapes later on.
...
To finally draw our first shape, we will use the method [draw_polygon](../godot_csharp_misc.md) and pass the points (as an array of Vector2 coordinates) and its color, like this:
...
And finally we will use the method [draw_polyline](../godot_csharp_misc.md) to actually draw the line, like this:
...
Notice how we can define colors in different ways, either with a hexadecimal code or a predefined color name. Check the class [Color](../godot_csharp_math_types.md) for other constants and ways to define Colors.

#### Drawing circles

To create the eyes, we are going to add 4 additional calls to draw the eye shapes, in different sizes, colors and positions.

To draw a circle, you position it based on its center using the [draw_circle](../godot_csharp_misc.md) method. The first parameter is a [Vector2](../godot_csharp_math_types.md) with the coordinates of its center, the second is its radius, and the third is its color:
...
For partial, unfilled arcs (portions of a circle shape between certain arbitrary angles), you can use the method [draw_arc](../godot_csharp_misc.md).

#### Drawing lines

To draw the final shape (the nose) we will use a line to approximate it.

[draw_line](../godot_csharp_misc.md) can be used to draw a single segment by providing its start and end coordinates as arguments, like this:
...
Note that if multiple unconnected lines are going to be drawn at the same time, you may get additional performance by drawing all of them in a single call, using the [draw_multiline](../godot_csharp_misc.md) method.

#### Drawing text

While using the [Label](../godot_csharp_ui_controls.md) Node is the most common way to add text to your application, the low-level \_draw function includes functionality to add text to your custom Node drawing. We will use it to add the name "GODOT" under the robot head.

We will use the [draw_string](../godot_csharp_misc.md) method to do it, like this:
...
Additional parameters as well as other methods related to text and characters can be found on the [CanvasItem](../godot_csharp_nodes_2d.md) class reference.
...
One possible way to work around this is to use the lower level [draw_set_transform](../godot_csharp_misc.md) method to fix this issue, translating all points in the CanvasItem's own space, and then moving it back to its original place with a regular node transform, either in the editor or in code, like this:
...
Please note that `_mouth_width` is a user defined property like any other and it or any other used as a drawing argument can be animated using more standard and high-level methods such as a [Tween](../godot_csharp_animation.md) or an [AnimationPlayer](../godot_csharp_resources.md) Node. The only difference is that a `queue_redraw()` call is needed to apply those changes so they get shown on screen.

### Example 2: drawing a dynamic line

The previous example was useful to learn how to draw and modify nodes with custom shapes and animations. This could have some advantages, such as using exact coordinates and vectors for drawing, rather than bitmaps -which means they will scale well when transformed on screen. In some cases, similar results could be achieved composing higher level functionality with nodes such as [sprites](../godot_csharp_misc.md) or [AnimatedSprites](../godot_csharp_misc.md) loading SVG resources (which are also images defined with vectors) and the [AnimationPlayer](../godot_csharp_resources.md) node.
...
In this example we obtain the position of the mouse in the default viewport every frame with the method [get_mouse_position](../godot_csharp_misc.md). If the position has changed since the last draw request (a small optimization to avoid redrawing on every frame)- we will schedule a redraw. Our `_draw()` method only has one line: requesting the drawing of a green line of width 10 pixels between the top left corner and that obtained position.
...
To draw the arc, we can use the method [draw_arc](../godot_csharp_misc.md). There are many arcs that pass through 2 points, so we will chose for this example the semicircle that has its center in the middle point between the 2 initial points.
...
[CanvasItem](../godot_csharp_nodes_2d.md) is the base node for 2D. [Node2D](../godot_csharp_nodes_2d.md) is the base node for 2D game objects, and [Control](../godot_csharp_ui_controls.md) is the base node for everything GUI. For 3D, Godot uses the [Node3D](../godot_csharp_nodes_3d.md) node.

### Displaying 3D nodes in 2D

It is possible to display 3D nodes in a 2D scene by using a [SubViewport](../godot_csharp_rendering.md). You can see this in the demo [3D in 2D Viewport](https://godotengine.org/asset-library/asset/2804).
