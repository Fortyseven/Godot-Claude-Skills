...
> **Warning:** When physical light units are enabled and if you have a WorldEnvironment node in your scene (i.e. the editor Environment is disabled), you **must** have a [CameraAttributes](../godot_csharp_rendering.md) resource assigned to the WorldEnvironment node. Otherwise, the 3D editor viewport will appear extremely bright if you have a visible DirectionalLight3D node.

On the Camera3D node, you can add a [CameraAttributes](../godot_csharp_rendering.md) resource to its **Attributes** property. This resource is used to control the camera's depth of field and exposure. When using [CameraAttributesPhysical](../godot_csharp_rendering.md), its focal length property is also used to adjust the camera's field of view.
...
If you haven't added a [WorldEnvironment](../godot_csharp_nodes_3d.md) and [Camera3D](../godot_csharp_nodes_3d.md) node to the current scene yet, do so now by clicking the 3 vertical dots at the top of the 3D editor viewport. Click **Add Sun to Scene**, open the dialog again then click **Add Environment to Scene**.

After enabling physical light units, a new property becomes available to edit in the [Environment](../godot_csharp_rendering.md) resource:
...
This tutorial will present the basics of using an [ArrayMesh](../godot_csharp_rendering.md).

To do so, we will use the function [add_surface_from_arrays()](../godot_csharp_misc.md), which takes up to five parameters. The first two are required, while the last three are optional.

The first parameter is the `PrimitiveType`, an OpenGL concept that instructs the GPU how to arrange the primitive based on the vertices given, i.e. whether they represent triangles, lines, points, etc. See [Mesh.PrimitiveType](../godot_csharp_misc.md) for the options available.
...
Common elements of `arrays` are listed below, together with the position they must have within `arrays`. See [Mesh.ArrayType](../godot_csharp_misc.md) for a full list.
...
For normal usage, the last three parameters in [add_surface_from_arrays()](../godot_csharp_misc.md) are typically left empty.

### Setting up the ArrayMesh

In the editor, create a [MeshInstance3D](../godot_csharp_nodes_3d.md) and add an [ArrayMesh](../godot_csharp_rendering.md) to it in the Inspector. Normally, adding an ArrayMesh in the editor is not useful, but in this case it allows us to access the ArrayMesh from code without creating one.
...
public partial class MyMeshInstance3D : MeshInstance3D
{
    public override void _Ready()
    {
        ...
        * *********************************/
        ...
        surfaceArray[(int)Mesh.ArrayType.Vertex] = verts.ToArray();
        surfaceArray[(int)Mesh.ArrayType.TexUV] = uvs.ToArray();
        surfaceArray[(int)Mesh.ArrayType.Norm
// ...
```
...
Finally, we can use the [ResourceSaver](../godot_csharp_core.md) class to save the ArrayMesh. This is useful when you want to generate a mesh and then use it later without having to re-generate it.
...
The [ImmediateMesh](../godot_csharp_rendering.md) is a convenient tool to create dynamic geometry using an OpenGL 1.x-style API. Which makes it both approachable to use and efficient for meshes which need to be updated every frame.
...
First, you need to create a [MeshInstance3D](../godot_csharp_nodes_3d.md) and add an [ImmediateMesh](../godot_csharp_rendering.md) to it in the Inspector.
...
The [MeshDataTool](../godot_csharp_rendering.md) is not used to generate geometry. But it is helpful for dynamically altering geometry, for example if you want to write a script to tessellate, simplify, or deform meshes.

The MeshDataTool is not as fast as altering arrays directly using [ArrayMesh](../godot_csharp_rendering.md). However, it provides more information and tools to work with meshes than the ArrayMesh does. When the MeshDataTool is used, it calculates mesh data that is not available in ArrayMeshes such as faces and edges, which are necessary for certain mesh algorithms. If you do not need this extra information then it may be better to use an ArrayMesh.

> **Note:** MeshDataTool can only be used on Meshes that use the PrimitiveType `Mesh.PRIMITIVE_TRIANGLES`.

We initialize the MeshDataTool from an ArrayMesh by calling [create_from_surface()](../godot_csharp_misc.md). If there is already data initialized in the MeshDataTool, calling `create_from_surface()` will clear it for you. Alternatively, you can call [clear()](../godot_csharp_misc.md) yourself before re-using the MeshDataTool.
...
These modifications are not done in place on the ArrayMesh. If you are dynamically updating an existing ArrayMesh, first delete the existing surface before adding a new one using [commit_to_surface()](../godot_csharp_misc.md):
...
The [SurfaceTool](../godot_csharp_rendering.md) provides a useful interface for constructing geometry. The interface is similar to the [ImmediateMesh](../godot_csharp_rendering.md) class. You set each per-vertex attribute (e.g. normal, uv, color) and then when you add a vertex it captures the attributes.
...
When finished generating your geometry with the [SurfaceTool](../godot_csharp_rendering.md), call `commit()` to finish generating the mesh. If an [ArrayMesh](../godot_csharp_rendering.md) is passed to `commit()`, then it appends a new surface to the end of the ArrayMesh. While if nothing is passed in, `commit()` returns an ArrayMesh.
...
// Alternatively we can use `st.Index()` which will create the quad for u
// ...
```
...
The rendering scale can be adjusted at runtime by changing the `scaling_3d_scale` property on a [Viewport](../godot_csharp_rendering.md) node.
...
The texture LOD bias can manually be changed by adjusting the **Rendering > Textures > Default Filters > Texture Mipmap Bias** advanced project setting. It can also be changed at runtime on [Viewports](../godot_csharp_rendering.md) by adjusting the `texture_mipmap_bias` property.
...
In Godot, this can be done by setting a [Camera3D](../godot_csharp_nodes_3d.md) as a child of a node. However, if you try this without any extra steps, you'll notice that the camera clips through geometry and hides the scene.

This is where the [SpringArm3D](../godot_csharp_nodes_3d.md) node comes in.
