# Godot 4 C# Tutorials — Scripting (Part 6)
...
In order to make an actual node, first we'll create a C struct to hold data and functions that will act as methods. The plan is to make this a custom node that inherits from [Sprite2D](../godot_csharp_nodes_2d.md).
...
Our custom node now has properties to influence how it operates, but it still doesn't do anything. In this section, we will bind the virtual method [\_process()](../godot_csharp_misc.md) and make our custom sprite move a little bit.
...
First, let's add [Vector2](../godot_csharp_math_types.md) to our `defs.h` file, so we can use it in our method:
...
In order to emit a signal, we need to call the [emit_signal()](../godot_csharp_misc.md) method on our custom node. Since this is a `vararg` function (meaning it takes any amount of arguments), we cannot use `ptrcall`. To do a regular call, we have to create Variants, which require a few more steps of plumbing to get done.
