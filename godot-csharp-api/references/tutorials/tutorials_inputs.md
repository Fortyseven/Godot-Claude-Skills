...
Use the method [Input.set_joy_light()](../godot_csharp_misc.md) to set the color of a given controller's LEDs.

To determine if a given controller supports setting LED lights, use the [Input.has_joy_light()](../godot_csharp_misc.md) method. The PlayStation DualShock and DualSense controllers are known to support LED lights.
...
Use the Input singleton's [start_joy_vibration](../godot_csharp_misc.md) method to start vibrating a gamepad. Use [stop_joy_vibration](../godot_csharp_misc.md) to stop vibration early (useful if no duration was specified when starting).

On mobile devices, you can also use [vibrate_handheld](../godot_csharp_misc.md) to vibrate the device itself (independently from the gamepad). On Android, this requires the `VIBRATE` permission to be enabled in the Android export preset before exporting the project.
...
If you want controller buttons to send echo events, you will have to generate [InputEvent](../godot_csharp_input.md) objects by code and parse them using [Input.parse_input_event()](../godot_csharp_misc.md) at regular intervals. This can be accomplished with the help of a [Timer](../godot_csharp_misc.md) node.
...
To test mappings on non-desktop platforms or to distribute your project with additional controller mappings, you can add them by calling [Input.add_joy_mapping()](../godot_csharp_misc.md) as early as possible in a script's `_ready()` function.
...
Open the **Project Settings** and go to **Display > Mouse Cursor**. You will see the settings [Custom Image](../godot_csharp_misc.md), [Custom Image Hotspot](../godot_csharp_misc.md), and [Tooltip Position Offset](../godot_csharp_ui_controls.md).
...
> **See also:** Check [Input.set_custom_mouse_cursor()](../godot_csharp_misc.md)'s documentation for more information on usage and platform-specific caveats.

### Cursor list

There are multiple mouse cursors you can define, documented in the [Input.CursorShape](../godot_csharp_misc.md) enum. Which ones you want to use depends on your use case.
...
On desktop and web platforms, [Node](../godot_csharp_core.md) receives a special `NOTIFICATION_WM_CLOSE_REQUEST` notification when quitting is requested from the window manager.
...
While forcing the application to close can be done by calling [SceneTree.quit](../godot_csharp_core.md), doing so will not send the `NOTIFICATION_WM_CLOSE_REQUEST` to the nodes in the scene tree. Quitting by calling [SceneTree.quit](../godot_csharp_core.md) will not allow custom actions to complete (such as saving, confirming the quit, or debugging), even if you try to delay the line that forces the quit.
...
Sending this notification will inform all nodes about the program termination, but will not terminate the program itself _unlike in 3.X_. In order to achieve the previous behavior, [SceneTree.quit](../godot_csharp_core.md) should be called after the notification.
...
Input events are objects that inherit from [InputEvent](../godot_csharp_input.md). Depending on the event type, the object will contain specific properties related to that event. To see what events actually look like, add a Node and attach the following script:
...
As you can see, the results are very different for the different types of input. Key events are even printed as their key symbols. For example, let's consider [InputEventMouseButton](../godot_csharp_input.md). It inherits from the following classes:

- [InputEvent](../godot_csharp_input.md) - the base class for all input events
- [InputEventWithModifiers](../godot_csharp_input.md) - adds the ability to check if modifiers are pressed, such as Shift or Alt.
- [InputEventMouse](../godot_csharp_input.md) - adds mouse event properties, such as `position`
- [InputEventMouseButton](../godot_csharp_input.md) - contains the index of the button that was pressed, whether it was a double-click, etc.
...
The [InputMap](../godot_csharp_input.md) is the most flexible way to handle a variety of inputs. You use this by creating named input _actions_, to which you can assign any number of input events, such as keypresses or mouse clicks. To see them, and to add your own, open Project -> Project Settings and select the InputMap tab:
...
Keyboard events are captured in [InputEventKey](../godot_csharp_input.md). While it's recommended to use input actions instead, there may be cases where you want to specifically look at key events. For this example, let's check for the T:
...
Modifier properties are inherited from [InputEventWithModifiers](../godot_csharp_input.md). This allows you to check for modifier combinations using boolean properties. Let's imagine you want one thing to happen when the T is pressed, but something different when it's Shift + T:
...
Mouse events stem from the [InputEventMouse](../godot_csharp_input.md) class, and are separated into two types: [InputEventMouseButton](../godot_csharp_input.md) and [InputEventMouseMotion](../godot_csharp_input.md). Note that this means that all mouse events will contain a `position` property.
...
[InputEventMouseMotion](../godot_csharp_input.md) events occur whenever the mouse moves. You can find the move's distance with the `relative` property.

Here's an example using mouse events to drag-and-drop a [Sprite2D](../godot_csharp_nodes_2d.md) node:
...
If you are using a touchscreen device, you can generate touch events. [InputEventScreenTouch](../godot_csharp_input.md) is equivalent to a mouse click event, and [InputEventScreenDrag](../godot_csharp_input.md) works much the same as mouse motion.
...
Managing input is usually complex, no matter the OS or platform. To ease this a little, a special built-in type is provided, [InputEvent](../godot_csharp_input.md). This datatype can be configured to contain several types of input events. Input events travel through the engine and can be received in multiple locations, depending on the purpose.
...
Every input event is originated from the user/player (though it's possible to generate an InputEvent and feed them back to the engine, which is useful for gestures). The DisplayServer for each platform will read events from the operating system, then feed them to the root [Window](../godot_csharp_ui_controls.md).

The window's [Viewport](../godot_csharp_rendering.md) does quite a lot of stuff with the received input, in order:
...
3. First of all, the standard [Node.\_input()](../godot_csharp_misc.md) function will be called in any node that overrides it (and hasn't disabled input processing with [Node.set_process_input()](../godot_csharp_misc.md)). If any function consumes the event, it can call [Viewport.set_input_as_handled()](../godot_csharp_rendering.md), and the event will not spread any more. This ensures that you can filter all events of interest, even before the GUI. For gameplay input, [Node.\_unhandled_input()](../godot_csharp_misc.md) is generally a better fit, because it allows the GUI to intercept the events.
4. Second, it will try to feed the input to the GUI, and see if any control can receive it. If so, the [Control](../godot_csharp_ui_controls.md) will be called via the virtual function [Control.\_gui_input()](../godot_csharp_ui_controls.md) and the signal "gui_input" will be emitted (this function is re-implementable by script by inheriting from it). If the control wants to "consume" the event, it will call [Control.accept_event()](../godot_csharp_ui_controls.md) and the event will not spread any more. Use the [Control.mouse_filter](../godot_csharp_ui_controls.md) property to control whether a [Control](../godot_csharp_ui_controls.md) is notified of mouse events via [Control.\_gui_input()](../godot_csharp_ui_controls.md) callback, and whether these events are propagated further.
5. If so far no one consumed the event, the [Node.\_shortcut_input()](../godot_csharp_misc.md) callback will be called if overridden (and not disabled with [Node.set_process_shortcut_input()](../godot_csharp_misc.md)). This happens only for [InputEventKey](../godot_csharp_input.md), [InputEventShortcut](../godot_csharp_input.md) and [InputEventJoypadButton](../godot_csharp_input.md). If any function consumes the event, it can call [Viewport.set_input_as_handled()](../godot_csharp_rendering.md), and the event will not spread any more. The shortcut input callback is ideal for treating events that are intended as shortcuts.
6. If so far no one consumed the event, the [Node.\_unhandled_key_input()](../godot_csharp_misc.md) callback will be called if overridden (and not disabled with [Node.set_process_unhandled_key_input()](../godot_csharp_misc.md)). This happens only if the event is an [InputEventKey](../godot_csharp_input.md). If any function consumes the event, it can call [Viewport.set_input_as_handled()](../godot_csharp_rendering.md), and the event will not spread any more. The unhandled key input callback is ideal for key events.
7. If so far no one consumed the event, the [Node.\_unhandled_input()](../godot_csharp_misc.md) callback will be called if overridden (and not disabled with [Node.set_process_unhandled_input()](../godot_csharp_misc.md)). If any function consumes the event, it can call [Viewport.set_input_as_handled()](../godot_csharp_rendering.md), and the event will not spread any more. The unhandled input callback is ideal for full-screen gameplay events, so they are not received when a GUI is active.
8. If no one wanted the event so far, and [Object Picking](../godot_csharp_core.md) is turned on, the event is used for object picking. For the root viewport, this can also be enabled in [Project Settings](../godot_csharp_misc.md). In the case of a 3D scene if a [Camera3D](../godot_csharp_nodes_3d.md) is assigned to the Viewport, a ray to the physics world (in the ray direction from the click) will be cast. If this ray hits an object, it will call the [CollisionObject3D.\_input_event()](../godot_csharp_misc.md) function in the relevant physics object. In the case of a 2D scene, conceptually the same happens with [CollisionObject2D.\_input_event()](../godot_csharp_nodes_2d.md).
...
> **Note:** This order doesn't apply to [Control.\_gui_input()](../godot_csharp_ui_controls.md), which uses a different method based on event location or focused Control. GUI **mouse** events also travel up the scene tree, subject to the [Control.mouse_filter](../godot_csharp_ui_controls.md) restrictions described above. However, since these events target specific Controls, only direct ancestors of the targeted Control node receive the event. GUI **keyboard and joypad** events _do not_ travel up the scene tree, and can only be handled by the Control that received them. Otherwise, they will be propagated as non-GUI events through [Node.\_unhandled_input()](../godot_csharp_misc.md).

Since Viewports don't send events to other [SubViewports](../godot_csharp_rendering.md), one of the following methods has to be used:

1. Use a [SubViewportContainer](../godot_csharp_ui_controls.md), which automatically sends events to its child [SubViewports](../godot_csharp_rendering.md) after [Node.\_input()](../godot_csharp_misc.md) or [Control.\_gui_input()](../godot_csharp_ui_controls.md).
...
[InputEvent](../godot_csharp_input.md) is just a base built-in type, it does not represent anything and only contains some basic information, such as event ID (which is increased for each event), device index, etc.
...
Any event has the methods [InputEvent.is_action()](../godot_csharp_input.md), [InputEvent.is_pressed()](../godot_csharp_input.md) and [InputEvent.is_echo()](../godot_csharp_input.md).

Alternatively, it may be desired to supply the game back with an action from the game code (a good example of this is detecting gestures). The Input singleton has a method for this: [Input.parse_input_event()](../godot_csharp_misc.md). You would normally use it like this:
