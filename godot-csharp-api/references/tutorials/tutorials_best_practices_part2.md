# Godot 4 C# Tutorials — Best Practices (Part 2)
...
3. Initialize a [Callable](../godot_csharp_misc.md) property. Safer than a method as ownership of the method is unnecessary. Used to start behavior.
...
> **Note:** Although the examples above illustrate parent-child relationships, the same principles apply towards all object relations. Nodes which are siblings should only be aware of their own hierarchies while an ancestor mediates their communications and references. ```csharp
> // Parent
...
> **Note:** In some cases, you need these separated nodes to *also* position themselves relative to each other. You can use the [RemoteTransform](../godot_csharp_misc.md) / [RemoteTransform2D](../godot_csharp_nodes_2d.md) nodes for this purpose. They will allow a target node to conditionally inherit selected transform elements from the Remote* node. To assign the `target` [NodePath](../godot_csharp_misc.md), use one of the following: 1. A reliable third party, likely a parent node, to mediate the assignment.
...
3. No opportunity for errors to occur because these details are not accounted for. In contrast, if you ever need a child node that does *not* inherit the transform of its parent, you have the following options: 1. The **declarative** solution: place a [Node](../godot_csharp_core.md) in between them. Since it doesn't have a transform, they won't pass this information to its children.
2. The **imperative** solution: Use the `top_level` property for the [CanvasItem](../godot_csharp_nodes_2d.md) or [Node3D](../godot_csharp_nodes_3d.md) node. This will make the node ignore its inherited transform.
...
- Set up using [EditorPlugin.add_custom_type](../godot_csharp_editor.md).
...
Scenes help to avoid this performance issue. [PackedScene](../godot_csharp_resources.md), the base type that scenes inherit from, defines resources that use serialized data to create objects. The engine can process scenes in batches on the back-end and provide much better performance than scripts.
...
The engine provides built-in classes like [Node](../godot_csharp_core.md). You can extend those to create derived types using a script.
...
Godot's internal classes have methods that register a class's data with a [ClassDB](../godot_csharp_core.md). This database provides runtime access to class information. `ClassDB` contains information about classes like:
...
Attaching a [Script](../godot_csharp_resources.md) to your object extends the methods, properties, and signals available from the `ClassDB`.

> **Note:** Even scripts that don't use the `extends` keyword implicitly inherit from the engine's base [RefCounted](../godot_csharp_core.md) class. As a result, you can instantiate scripts without the `extends` keyword from code. Since they extend `RefCounted` though, you cannot attach them to a [Node](../godot_csharp_core.md).
