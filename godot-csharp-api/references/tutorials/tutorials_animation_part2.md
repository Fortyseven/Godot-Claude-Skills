# Godot 4 C# Tutorials — Animation (Part 2)
...
- **Custom Shaped Elements**: Custom shapes can be created with [Polygon2D](../godot_csharp_nodes_2d.md) allowing UV animation, deformations, etc.
...
This small cross in the middle of the [Sprite2D](../godot_csharp_nodes_2d.md) is the rotation pivot:
...
The [RemoteTransform2D](../godot_csharp_nodes_2d.md) node transforms nodes somewhere else in the hierarchy. This node applies its own transform (including any transformation it inherits from its parents) to the remote node it targets.
...
The first step is creating an endpoint node. Any kind of node will do, but [Marker2D](../godot_csharp_nodes_2d.md) is preferred because it's visible in the editor. The endpoint node will ensure that the last bone has orientation.
...
The [AnimationPlayer](../godot_csharp_resources.md) node allows you to create anything from simple to complex animations.
...
To use the animation tools we first have to create an [AnimationPlayer](../godot_csharp_resources.md) node.
...
To play the part of the animation between two markers use the [play_section_with_markers()](../godot_csharp_misc.md) and [play_section_with_markers_backwards()](../godot_csharp_misc.md) methods. If no start marker is specified then the beginning of the animation is used, and if no end marker is specified, then the end of the animation is used.
...
Godot supports video playback with the [VideoStreamPlayer](../godot_csharp_ui_controls.md) node.
...
Using a VideoStreamPlayer node as a child of a [SubViewport](../godot_csharp_rendering.md) node, it's possible to display any 2D node on a 3D surface. For example, this can be used to display animated billboards when frame-by-frame animation would require too much memory.

This can be done with the following steps:

1. Create a [SubViewport](../godot_csharp_rendering.md) node. Set its size to match your video's size in pixels.
