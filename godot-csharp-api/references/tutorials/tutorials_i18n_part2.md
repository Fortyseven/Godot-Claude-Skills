# Godot 4 C# Tutorials — I18N (Part 2)
...
Pseudolocalization can be toggled at runtime using the [pseudolocalization_enabled](../godot_csharp_misc.md) property in TranslationServer. However, if runtime configuration of pseudolocalization properties is required, they can be directly configured using [ProjectSettings.set_setting(property, value)](../godot_csharp_filesystem.md) and then calling [TranslationServer.reload_pseudolocalization()](../godot_csharp_misc.md) which reparses the pseudolocalization properties and reloads the pseudolocalization. The following code snippet shall turn on `replace_with_accents` and `double_vowels` properties and then call `reload_pseudolocalization()` for the changes to get reflected:
