# Godot 4 C# Tutorials — Animation (Part 1)
...
A blend shape track is optimized for animating blend shape in [MeshInstance3D](../godot_csharp_nodes_3d.md).
...
> **Note:** Bezier curve track and property track cannot be blended in [AnimationPlayer](../godot_csharp_resources.md) and [AnimationTree](../godot_csharp_resources.md).
...
The blend mode allows you to choose whether or not to adjust the audio volume when blending in the [AnimationTree](../godot_csharp_resources.md).
...
With [AnimationPlayer](../godot_csharp_resources.md), Godot has one of the most flexible animation systems that you can find in any game engine. It is pretty much unique in its ability to animate almost any property in any node or resource, and its dedicated transform, bezier, function calling, audio, and sub-animation tracks.
...
[AnimationTree](../godot_csharp_resources.md) is a node designed to deal with advanced transitions.
...
> **See also:** The Advance Expression is evaluated using Godot's [Expression](../godot_csharp_misc.md) class. See [Evaluating expressions](tutorials_scripting.md) for more information on writing expressions.
...
To use the travel ability, you should first retrieve the [AnimationNodeStateMachinePlayback](../godot_csharp_resources.md) object from the `AnimationTree` node (it is exported as a property), and then call one of its many functions:
...
Afterwards, the actual motion can be retrieved via the [AnimationTree](../godot_csharp_resources.md) API as a transform:
...
This can be fed to functions such as [CharacterBody3D.move_and_slide](../godot_csharp_nodes_3d.md) to control the character movement.
...
> **Note:** The `--write-movie` and `--fixed-fps` command line arguments are both available in exported projects. Movie Maker mode cannot be toggled while the project is running, but you can use the [OS.execute()](../godot_csharp_misc.md) method to run a second instance of the exported project that will record a video file.

### Choosing an output format

Output formats are provided by the [MovieWriter](../godot_csharp_misc.md) class. Godot has 3 built-in [MovieWriters](../godot_csharp_misc.md), and more can be implemented by extensions:
...
The resulting file can be viewed in Godot with [VideoStreamPlayer](../godot_csharp_ui_controls.md) and most video players but not web browsers. OGV does not support transparency.
