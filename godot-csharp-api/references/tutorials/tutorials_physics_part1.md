# Godot 4 C# Tutorials — Physics (Part 1)
...
You can define the shape of a [PhysicsBody2D](../godot_csharp_physics.md) by adding one or more [CollisionShape2Ds](../godot_csharp_nodes_2d.md) or [CollisionPolygon2Ds](../godot_csharp_nodes_2d.md) as _direct_ child nodes. Indirect child nodes (i.e. children of child nodes) will be ignored and won't be used as collision shapes. Also, note that you must add a [Shape2D](../godot_csharp_physics.md) _resource_ to collision shape nodes in the Inspector dock.
...
- [RectangleShape2D](../godot_csharp_misc.md)
- [CircleShape2D](../godot_csharp_misc.md)
- [CapsuleShape2D](../godot_csharp_misc.md)
- [SegmentShape2D](../godot_csharp_misc.md)
- [SeparationRayShape2D](../godot_csharp_misc.md) (designed for characters)
- [WorldBoundaryShape2D](../godot_csharp_misc.md) (infinite plane)
...
You can define the shape of a [PhysicsBody3D](../godot_csharp_physics.md) by adding one or more [CollisionShape3Ds](../godot_csharp_physics.md) as _direct_ child nodes. Indirect child nodes (i.e. children of child nodes) will be ignored and won't be used as collision shapes. Also, note that you must add a [Shape3D](../godot_csharp_physics.md) _resource_ to collision shape nodes in the Inspector dock.
...
- [BoxShape3D](../godot_csharp_misc.md)
- [SphereShape3D](../godot_csharp_misc.md)
- [CapsuleShape3D](../godot_csharp_misc.md)
- [CylinderShape3D](../godot_csharp_misc.md)
...
- In 3D, it is easy to turn interpolation on and off at the level of each `Node`, via the [physics_interpolation_mode](../godot_csharp_misc.md) property in the Inspector, which can be set to `On`, `Off`, or `Inherited`.
...
Even with physics interpolation active, there may be some local situations where you would benefit from disabling automatic interpolation for a [Node](../godot_csharp_core.md) (or branch of the [SceneTree](../godot_csharp_core.md)), and have the finer control of performing interpolation manually.

This is possible using the [Node.physics_interpolation_mode](../godot_csharp_misc.md) property which is present in all Nodes. If you for example, turn off interpolation for a Node, the children will recursively also be affected (as they default to inheriting the parent setting). This means you can easily disable interpolation for an entire subscene.

The most common situation where you may want to perform your own interpolation is Cameras.

#### Cameras

In many cases, a [Camera3D](../godot_csharp_nodes_3d.md) can use automatic interpolation just like any other node. However, for best results, especially at low physics tick rates, it is recommended that you take a manual approach to camera interpolation.
...
1. Call [Node3D.top_level](../godot_csharp_nodes_3d.md) and set this to `true`, which will make the Camera ignore the transform of its parent.
...
We can do this using the [Node3D.get_global_transform_interpolated](../godot_csharp_nodes_3d.md) function. This acts exactly like getting [Node3D.global_transform](../godot_csharp_nodes_3d.md) but it gives you the _interpolated_ transform (during a `_process()` call).
...
In this situation, it can be better to disable physics interpolation for the camera node (using [Node.physics_interpolation_mode](../godot_csharp_misc.md)) and directly apply the mouse input to the camera rotation, rather than apply it in `_physics_process`.

Sometimes, especially with cameras, you will want to use a combination of interpolation and non-interpolation:

- A first person camera may position the camera at a player location (perhaps using [Node3D.get_global_transform_interpolated](../godot_csharp_nodes_3d.md)), but control the Camera rotation from mouse look _without_ interpolation.
- A third person camera may similarly determine the look at (target location) of the camera using [Node3D.get_global_transform_interpolated](../godot_csharp_nodes_3d.md), but position the camera using mouse look _without_ interpolation.
...
- [MultiMesh.reset_instance_physics_interpolation](../godot_csharp_rendering.md)
- [MultiMesh.set_buffer_interpolated](../godot_csharp_rendering.md)

Full details are in the [MultiMesh](../godot_csharp_rendering.md) documentation.
...
One key concept to understand in Godot is the distinction between physics ticks (sometimes referred to as iterations or physics frames), and rendered frames. The physics proceeds at a fixed tick rate (set in [Project Settings > Physics > Common > Physics Tick per Second](../godot_csharp_misc.md)), which defaults to 60 ticks per second.
...
This is called the **physics interpolation fraction**, and is handily calculated for you by Godot. It can be retrieved on any frame by calling [Engine.get_physics_interpolation_fraction](../godot_csharp_core.md).
...
- Turn on physics interpolation: [Project Settings > Physics > Common > Physics Interpolation](../godot_csharp_misc.md)
...
- Be sure to call [Node.reset_physics_interpolation](../godot_csharp_misc.md) on nodes _after_ you first position or teleport them, to prevent "streaking".
- Temporarily try setting [Project Settings > Physics > Common > Physics Ticks per Second](../godot_csharp_misc.md) to 10 to see the difference with and without interpolation.
...
The first step is to turn on physics interpolation in [Project Settings > Physics > Common > Physics Interpolation](../godot_csharp_misc.md) You can now run your game.
...
> **Tip:** To convert an existing game to use interpolation, it is highly recommended that you temporarily set [Project Settings > Physics > Common > Physics Tick per Second](../godot_csharp_misc.md) to a low value such as `10`, which will make interpolation problems more obvious.
...
> **Note:** Also consider that nodes can be moved not just by moving themselves, but also by moving parent nodes in the [SceneTree](../godot_csharp_core.md). The movement of parents should therefore also only occur during physics ticks.
...
The gameplay may not work perfectly, but it should enable you to more easily see cases where you should be calling [Node.reset_physics_interpolation](../godot_csharp_misc.md), or where you should be using your own custom interpolation on e.g. a [Camera3D](../godot_csharp_nodes_3d.md). Once you have these cases fixed, you can set the physics tick rate back to the desired setting.
...
Go back to the character scene, and open the script, the magic begins now! Kinematic body will do nothing by default, but it has a useful function called `CharacterBody2D.move_and_collide()`. This function takes a [Vector2](../godot_csharp_math_types.md) as an argument, and tries to apply that motion to the kinematic body. If a collision happens, it stops right at the moment of the collision.
