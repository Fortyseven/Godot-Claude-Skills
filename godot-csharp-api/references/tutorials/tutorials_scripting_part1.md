# Godot 4 C# Tutorials — Scripting (Part 1)
...
> **Tip:** If you need a dictionary where the key is typed but not the value, use `Variant` as the `TValue` generic parameter of the typed dictionary. ```csharp
...
`Godot.Variant` is used to represent Godot's native [Variant](../godot_csharp_misc.md) type. Any Variant-compatible type can be converted from/to it.
