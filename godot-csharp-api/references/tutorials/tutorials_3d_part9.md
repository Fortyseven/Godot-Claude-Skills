...
In our example, we would use a [Node3D](../godot_csharp_nodes_3d.md) node as the root node of the scene. Your scene tree would look like this:
...
The result of all this is that you should **not use** the `rotation` property of [Node3D](../godot_csharp_nodes_3d.md) nodes in Godot for games. It's there to be used mainly in the editor, for coherence with the 2D engine, and for simple rotations (generally just one axis, or even two in limited cases). As much as you may be tempted, don't use it.

Instead, there is a better way to solve your rotation problems.

### Introducing transforms

Godot uses the [Transform3D](../godot_csharp_math_types.md) datatype for orientations. Each [Node3D](../godot_csharp_nodes_3d.md) node contains a `transform` property which is relative to the parent's transform, if the parent is a Node3D-derived type.

It is also possible to access the world coordinate transform via the `global_transform` property.

A transform has a [Basis](../godot_csharp_math_types.md) (transform.basis sub-property), which consists of three [Vector3](../godot_csharp_math_types.md) vectors. These are accessed via the `transform.basis` property and can be accessed directly by `transform.basis.x`, `transform.basis.y`, and `transform.basis.z`. Each vector points in the direction its axis has been rotated, so they effectively describe the node's total rotation. The scale (as long as it's uniform) can also be inferred from the length of the axes. A _basis_ can also be interpreted as a 3x3 matrix and used as `transform.basis[x][y]`.
...
For custom viewports, the VRS mode and texture must be set manually to the [Viewport](../godot_csharp_rendering.md) node.
...
> **Note:** The target of **Visibility Parent** _must_ inherit from [GeometryInstance3D](../godot_csharp_nodes_3d.md). Despite its name, the **Visibility Parent** property _can_ point to a node that is not a parent of the node in the scene tree. However, it is impossible to point **Visibility Parent** towards a child node, as this creates a dependency cycle which is not supported. You will get an error message in the Output panel if a dependency cycle occurs.
