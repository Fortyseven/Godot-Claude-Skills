# Godot 4 C# Tutorials — Shaders (Part 6)
...
This tutorial will introduce you to using the [SubViewport](../godot_csharp_rendering.md) as a texture that can be applied to 3D objects. In order to do so, it will walk you through the process of making a procedural planet like the one below:

> **Note:** This tutorial does not cover how to code a dynamic atmosphere like the one this planet has.

This tutorial assumes you are familiar with how to set up a basic scene including: a [Camera3D](../godot_csharp_nodes_3d.md), a [light source](../godot_csharp_misc.md), a [MeshInstance3D](../godot_csharp_nodes_3d.md) with a [Primitive Mesh](../godot_csharp_misc.md), and applying a [StandardMaterial3D](../godot_csharp_rendering.md) to the mesh. The focus will be on using the [SubViewport](../godot_csharp_rendering.md) to dynamically create textures that can be applied to the mesh.

In this tutorial, we'll cover the following topics:

- How to use a [SubViewport](../godot_csharp_rendering.md) as a render texture
- Mapping a texture to a sphere with equirectangular mapping
- Fragment shader techniques for procedural planets
- Setting a Roughness map from a [Viewport Texture](../godot_csharp_rendering.md)
...
Click on the [SubViewport](../godot_csharp_rendering.md) node and set its size to `(1024, 512)`. The [SubViewport](../godot_csharp_rendering.md) can actually be any size so long as the width is double the height. The width needs to be double the height so that the image will accurately map onto the sphere, as we will be using equirectangular projection, but more on that later.

Next disable 3D. We will be using a [ColorRect](../godot_csharp_ui_controls.md) to render the surface, so we don't need 3D either.

Select the [ColorRect](../godot_csharp_ui_controls.md) and in the inspector set the anchors preset to `Full Rect`. This will ensure that the [ColorRect](../godot_csharp_ui_controls.md) takes up the entire [SubViewport](../godot_csharp_rendering.md).

Next, we add a [Shader Material](../godot_csharp_rendering.md) to the [ColorRect](../godot_csharp_ui_controls.md) (ColorRect > CanvasItem > Material > Material > `New ShaderMaterial`).
...
Now we have the basics of a [SubViewport](../godot_csharp_rendering.md) that we render to and we have a unique image that we can apply to the sphere.

### Applying the texture

Now go into the [MeshInstance3D](../godot_csharp_nodes_3d.md) and add a [StandardMaterial3D](../godot_csharp_rendering.md) to it. No need for a special [Shader Material](../godot_csharp_rendering.md) (although that would be a good idea for more advanced effects, like the atmosphere in the example above).
...
So now, when we render to our [SubViewport](../godot_csharp_rendering.md), it appears magically on the sphere. But there is an ugly seam created by our texture coordinates. So how do we get a range of coordinates that wrap around the sphere in a nice way? One solution is to use a function that repeats on the domain of our texture. `sin` and `cos` are two such functions. Let's apply them to the texture and see what happens. Replace the existing color code in the shader with the following:

```glsl
COLOR.xyz = vec3(sin(UV.x * 3.14159 * 4.0) * cos(UV.y * 3.14159 * 4.0) * 0.5 + 0.5);
```

Not too bad. If you look around, you can see that the seam has now disappeared, but in its place, we have pinching at the poles. This pinching is due to the way Godot maps textures to spheres in its [StandardMaterial3D](../godot_csharp_rendering.md). It uses a projection technique called equirectangular projection, which translates a spherical map onto a 2D plane.
...
Next, under the "Roughness" section set the roughness texture to a [Viewport Texture](../godot_csharp_rendering.md) pointing to our planet texture [SubViewport](../godot_csharp_rendering.md). Finally, set the `Texture Channel` to `Alpha`. This instructs the renderer to use the `alpha` channel of our output `COLOR` as the `Roughness` value.

You'll notice that very little changes except that the planet is no longer reflecting the sky. This is happening because, by default, when something is rendered with an alpha value, it gets drawn as a transparent object over the background. And since the default background of the [SubViewport](../godot_csharp_rendering.md) is opaque, the `alpha` channel of the [Viewport Texture](../godot_csharp_rendering.md) is `1`, resulting in the planet texture being drawn with slightly fainter colors and a `Roughness` value of `1` everywhere. To correct this, we go into the [SubViewport](../godot_csharp_rendering.md) and enable the "Transparent Bg" property. Since we are now rendering one transparent object on top of another, we want to enable `blend_premul_alpha`:
...
Now the planet should look like it is reflecting light on the ocean but not the land. move around the [OmniLight3D](../godot_csharp_nodes_3d.md) in the scene so you can see the effect of the reflections on the ocean.

And there you have it. A procedural planet generated using a [SubViewport](../godot_csharp_rendering.md).
...
VisualShaders can be created in any [ShaderMaterial](../godot_csharp_rendering.md). To begin using VisualShaders, create a new `ShaderMaterial` in an object of your choice.

Then assign a [Shader](../godot_csharp_rendering.md) resource to the `Shader` property.

Click on the new `Shader` resource and the Create Shader dialog will open automatically. Change the Type option to [VisualShader](../godot_csharp_rendering.md) in the dropdown, then give it a name.
...
In order to use a shader it must be attached inside a [Material](../godot_csharp_rendering.md) which must be attached to an object. Materials are a type of [Resource](tutorials_scripting.md). To draw multiple objects with the same material, the material must be attached to each object.

All objects derived from a [CanvasItem](../godot_csharp_nodes_2d.md) have a material property. This includes all [GUI elements](../godot_csharp_misc.md), [Sprite2Ds](../godot_csharp_nodes_2d.md), [TileMapLayers](../godot_csharp_nodes_2d.md), [MeshInstance2Ds](../godot_csharp_nodes_2d.md) etc. They also have an option to inherit their parent's material. This can be useful if you have a large number of nodes that you want to use the same material.
...
> **Note:** Materials that inherit from the [Material](../godot_csharp_rendering.md) resource, such as [StandardMaterial3D](../godot_csharp_rendering.md) and [ParticleProcessMaterial](../godot_csharp_rendering.md), can be converted to a [ShaderMaterial](../godot_csharp_rendering.md) and their existing properties will be converted to an accompanying text shader. To do so, right-click on the material in the FileSystem dock and choose **Convert to ShaderMaterial**. You can also do so by right-clicking on any property holding a reference to the material in the inspector.
...
You have decided to start writing your own custom Spatial shader. Maybe you saw a cool trick online that was done with shaders, or you have found that the [StandardMaterial3D](../godot_csharp_rendering.md) isn't quite meeting your needs. Either way, you have decided to write your own and now you need to figure out where to start.
...
In 3D, objects are drawn using [Meshes](../godot_csharp_misc.md). Meshes are a resource type that store geometry (the shape of your object) and materials (the color and how the object reacts to light) in units called "surfaces". A Mesh can have multiple surfaces, or just one. Typically, you would import a mesh from another program (e.g. Blender). But Godot also has a few [PrimitiveMeshes](../godot_csharp_rendering.md) that allow you to add basic geometry to a scene without importing Meshes.

There are multiple node types that you can use to draw a mesh. The main one is [MeshInstance3D](../godot_csharp_nodes_3d.md), but you can also use [GPUParticles3D](../godot_csharp_misc.md), [MultiMeshes](../godot_csharp_rendering.md) (with a [MultiMeshInstance3D](../godot_csharp_nodes_3d.md)), or others.
...
Add a new [MeshInstance3D](../godot_csharp_nodes_3d.md) node to your scene.

In the inspector tab, set the MeshInstance3D's **Mesh** property to a new [PlaneMesh](../godot_csharp_rendering.md) resource, by clicking on `<empty>` and choosing **New PlaneMesh**. Then expand the resource by clicking on the image of a plane that appears.
...
Now set **Subdivide Width** and **Subdivide Depth** of the [PlaneMesh](../godot_csharp_rendering.md) to `32`.

You can see that there are now many more triangles in the [MeshInstance3D](../godot_csharp_nodes_3d.md). This will give us more vertices to work with and thus allow us to add more detail.

[PrimitiveMeshes](../godot_csharp_rendering.md), like PlaneMesh, only have one surface, so instead of an array of materials there is only one. Set the **Material** to a new ShaderMaterial, then expand the material by clicking on the sphere that appears.
...
The `vertex()` function determines where the vertices of your [MeshInstance3D](../godot_csharp_nodes_3d.md) appear in the final scene. We will be using it to offset the height of each vertex and make our flat plane appear like a little terrain.
...
Godot provides the [NoiseTexture2D](../godot_csharp_resources.md) resource for generating a noise texture that can be accessed from a shader.
...
Set this **Noise** parameter to a new [NoiseTexture2D](../godot_csharp_resources.md). Then in your NoiseTexture2D, set its **Noise** property to a new [FastNoiseLite](../godot_csharp_resources.md). The FastNoiseLite class is used by the NoiseTexture2D to generate a heightmap.
...
Uniform variables allow you to pass data from the game into the shader. They are very useful for controlling shader effects. Uniforms can be almost any datatype that can be used in the shader. To use a uniform, you declare it in your [Shader](../godot_csharp_rendering.md) using the keyword `uniform`.
...
Using uniforms, we can even change the value every frame to animate the height of the terrain. Combined with [Tweens](../godot_csharp_misc.md), this can be especially useful for animations.
...
First, we will add an [OmniLight3D](../godot_csharp_nodes_3d.md) to the scene, and drag it up so it is above the terrain.

You can see the light affecting the terrain, but it looks odd. The problem is the light is affecting the terrain as if it were a flat plane. This is because the light shader uses the normals from the [Mesh](../godot_csharp_rendering.md) to calculate light.
...
Set this second uniform texture to another [NoiseTexture2D](../godot_csharp_resources.md) with another [FastNoiseLite](../godot_csharp_resources.md). But this time, check **As Normal Map**.
