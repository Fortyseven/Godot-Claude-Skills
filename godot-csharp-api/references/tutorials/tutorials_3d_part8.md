...
The filtering method for the textures used by the material. See [this page](../godot_csharp_misc.md) for a full list of options and their description.

#### Repeat

if the textures used by the material repeat, and how they repeat. See [this page](../godot_csharp_misc.md) for a full list of options and their description.
...
Objects are sorted by an opaque/transparent queue, then [render_priority](../godot_csharp_misc.md), with higher priority being drawn later. Transparent objects are also sorted by depth.

Depth testing overrules priority. Priority alone cannot force opaque objects to be drawn over each other.

### Next Pass

Setting [next_pass](../godot_csharp_misc.md) on a material will cause an object to be rendered again with that next material.

Materials are sorted by an opaque/transparent queue, then [render_priority](../godot_csharp_misc.md), with higher priority being drawn later.

Depth will test equal between both materials unless the grow setting or other vertex transformations are used. Multiple transparent passes should use [render_priority](../godot_csharp_misc.md) to ensure correct ordering.
...
[Gridmaps](../godot_csharp_misc.md) are a tool for creating 3D game levels, similar to the way [TileMap](tutorials_2d.md) works in 2D. You start with a predefined collection of 3D meshes (a [MeshLibrary](../godot_csharp_misc.md)) that can be placed on a grid, as if you were building a level with an unlimited amount of Lego blocks.
...
To begin, you need a [MeshLibrary](../godot_csharp_misc.md), which is a collection of individual meshes that can be used in the gridmap. Open the "mesh_library_source.tscn" scene to see an example of how to set up the mesh library.

As you can see, this scene has a [Node3D](../godot_csharp_nodes_3d.md) node as its root, and a number of [MeshInstance3D](../godot_csharp_nodes_3d.md) node children.
...
You can manually assign a [StaticBody3D](../godot_csharp_nodes_3d.md) and [CollisionShape3D](../godot_csharp_physics.md) to each mesh. Alternatively, you can use the "Mesh" menu to automatically create the collision body based on the mesh data.
...
Like all mesh instances, MeshLibrary items can be assigned a [NavigationMesh](../godot_csharp_misc.md) resource, which can be created manually, or baked as described below.

To create the NavigationMesh from a MeshLibrary scene export, place a [NavigationRegion3D](../godot_csharp_nodes_3d.md) child node below the main MeshInstance3D for the GridMap item. Add a valid NavigationMesh resource to the NavigationRegion3D and some source geometry nodes below and bake the NavigationMesh.
...
- Be a [MeshInstance3D](../godot_csharp_nodes_3d.md), which will become the MeshLibrary item. Only this visual mesh will be exported.
- Have a material, in the mesh's material slot, _not_ the MeshInstance3D's material slots.
- Have up to one [StaticBody3D](../godot_csharp_nodes_3d.md) child, for collision. The StaticBody3D should have one or more [CollisionShape3D](../godot_csharp_physics.md) children.
- Have up to one [NavigationRegion3D](../godot_csharp_nodes_3d.md) child, for navigation. The NavigationRegion3D can have one or more additional [MeshInstance3D](../godot_csharp_nodes_3d.md) children, which can be baked for navigation, but won't be exported as a visual mesh.
...
See [GridMap](../godot_csharp_misc.md) for details on the node's methods and member variables.
