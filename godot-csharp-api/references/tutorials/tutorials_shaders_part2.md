# Godot 4 C# Tutorials — Shaders (Part 2)
...
- In the `vertex()` function, `COLOR` contains the color from the vertex primitive multiplied by the CanvasItem's [modulate](../godot_csharp_misc.md) multiplied by the CanvasItem's [self_modulate](../godot_csharp_misc.md).
...
Certain nodes (for example, [Sprite2D](../godot_csharp_nodes_2d.md)) display a texture by default, for example [texture](../godot_csharp_misc.md). When using a custom `fragment()` function, you have a few options on how to sample this texture.
...
Similarly, if a normal map is used in the [CanvasTexture](../godot_csharp_misc.md), Godot uses it by default and assigns its value to the built-in `NORMAL` variable. If you are using a normal map meant for use in 3D, it will appear inverted. In order to use it in your shader, you must assign it to the `NORMAL_MAP` property. Godot will handle converting it for use in 2D and overwriting `NORMAL`.
...
The signed distance field is generated from [LightOccluder2D](../godot_csharp_nodes_2d.md) nodes present in the scene with the **SDF Collision** property enabled (which is the default). See the [2D lights and shadows](tutorials_2d.md) documentation for more information.
...
Fog shaders are used to define how fog is added to (or subtracted from) a scene in a given area. Fog shaders are always used together with [FogVolumes](../godot_csharp_nodes_3d.md) and volumetric fog. Fog shaders only have one processing function, the `fog()` function.

The resolution of the fog shaders depends on the resolution of the volumetric fog froxel grid. Accordingly, the level of detail that a fog shader can add depends on how close the [FogVolume](../godot_csharp_nodes_3d.md) is to the camera.

Fog shaders are a special form of compute shader that is called once for every froxel that is touched by an axis-aligned bounding box of the associated [FogVolume](../godot_csharp_nodes_3d.md). This means that froxels that just barely touch a given [FogVolume](../godot_csharp_nodes_3d.md) will still be used.
...
All of the output values of fog volumes overlap one another. This allows [FogVolumes](../godot_csharp_nodes_3d.md) to be rendered efficiently as they can all be drawn at once.
...
> **Note:** Particle shaders are only available with GPU-based particle nodes ([GPUParticles2D](../godot_csharp_nodes_2d.md) and [GPUParticles3D](../godot_csharp_misc.md)). CPU-based particle nodes ([CPUParticles2D](../godot_csharp_nodes_2d.md) and [CPUParticles3D](../godot_csharp_misc.md)) are _rendered_ on the GPU (which means they can use custom CanvasItem or Spatial shaders), but their motion is _simulated_ on the CPU.
