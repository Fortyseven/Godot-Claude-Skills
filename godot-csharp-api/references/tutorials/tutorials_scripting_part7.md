# Godot 4 C# Tutorials — Scripting (Part 7)
...
In GDScript, only base types (int, float, string and the vector types) are passed by value to functions (value is copied). Everything else (instances, arrays, dictionaries, etc) is passed as reference. Classes that inherit [RefCounted](../godot_csharp_core.md) (the default if nothing is specified) will be freed when not used, but manual memory management is allowed too if inheriting manually from [Object](../godot_csharp_core.md).
