# Godot 4 C# Tutorials — Shaders (Part 4)
...
You can create new shader includes by using the **File > Create Shader Include** menu option of the shader editor, or by creating a new [ShaderInclude](../godot_csharp_rendering.md) resource in the FileSystem dock.
...
Using `source_color` hint is required in the Forward+ and Mobile renderers, and in `canvas_item` shaders when [HDR 2D](../godot_csharp_misc.md) is enabled. The `source_color` hint is optional for the Compatibility renderer, and for `canvas_item` shaders if `HDR 2D` is disabled. However, it is recommended to always use the `source_color` hint, because it works even if you change renderers or disable `HDR 2D`.
...
To change the value of a global uniform at runtime, use the [RenderingServer.global_shader_parameter_set](../godot_csharp_rendering.md) method in a script:
...
Per-instance uniform values can also be set at runtime using [set_instance_shader_parameter](../godot_csharp_misc.md) method on a node that inherits from [GeometryInstance3D](../godot_csharp_nodes_3d.md):
...
You can set uniforms from GDScript using the [set_shader_parameter()](../godot_csharp_misc.md) method:
